...
const honorsData = JSON.parse(
  fs.readFileSync('enhanced-honors-complete.json', 'utf8')
)
...
// Write back to file
fs.writeFileSync(
  'enhanced-honors-complete-with-categories.json',
...
// Show some stats
const goldenGeekEntries = updatedData.filter(
  (h) => h.awardSet && h.awardSet.includes('Golden Geek')
)
