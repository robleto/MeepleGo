      ...
      <div className="grid grid-cols-5 gap-4">
        {ratingColors.map(({ value, color, label }) => (
          <div key={value} className="text-center">
            <div className={`h-16 w-16 rounded-lg bg-${color} flex items-center justify-center text-white font-bold shadow mx-auto mb-2`}>
              {value}
            </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex gap-2">
          {['gray-50', 'gray-100', 'gray-200', 'gray-300', 'gray-400', 'gray-500', 'gray-600', 'gray-700', 'gray-800', 'gray-900'].map(color => (
            <div key={color} className={`h-12 w-12 rounded bg-${color} border`} title={color} />
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          {['indigo-50', 'indigo-100', 'indigo-500', 'indigo-600', 'indigo-700'].map(color => (
            <div key={color} className={`h-12 w-12 rounded bg-${color} border`} title={color} />
          ))}
        </div>
      </div>
