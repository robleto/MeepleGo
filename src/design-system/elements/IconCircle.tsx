...
export const ICON_CIRCLE_TONES = ['neutral','primary','amber','pink','sky','emerald'] as const
...
export function IconCircle({ size='md', tone='neutral', border=true, children, className='' }: IconCircleProps) {
  return (
    <div className={`flex items-center justify-center rounded-full ${sizeMap[size]} ${toneMap[tone]} ${border? 'border border-current/20' : ''} ${className}`}>{children}</div>
  )
}
