...
const thumbs: Thumb[] = Array.from({ length: 5 }).map((_, i) => ({
  id: 'g' + i,
  name:
    ['Wingspan', 'Gloomhaven', 'Ark Nova', 'Brass: Birmingham', 'Azul'][i] ||
    'Game ' + i,
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'Atomic visual pattern: fanned stack of up to 5 game thumbnails used in ListCard header.',
      },
    },
  },
}
        ...
        {thumbs.slice(0, 4).map((g, index) => {
          ...
          return (
            ...
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={`https://placehold.co/96x120?text=${index + 1}`}
                alt={g.name}
