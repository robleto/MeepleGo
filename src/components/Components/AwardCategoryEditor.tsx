...
function NomineeItem({
  ...
  return (
    <li
      ref={setNodeRef}
      style={style}
      className={`group relative flex items-center gap-2 text-[11px] px-2 py-1 rounded border bg-gray-50 dark:bg-gray-800 dark:border-gray-700 cursor-grab ${isWinner ? 'ring-1 ring-primary-500' : ''}`}
      {...attributes}
      {...listeners}
      title={name}
    >
        ...
        {winnerId ? (
          <div className="flex items-center gap-2">
            {gameMap[winnerId]?.thumbnail_url && (
              ...
              <img
                src={gameMap[winnerId].thumbnail_url!}
                alt=""
      ...
      <div>
              ...
              <ul className="flex flex-col gap-1">
                {nominees.map((id) => (
                  <div key={id} className="relative group">
                    <NomineeItem
                      id={id}
                      name={gameMap[id]?.name || `#${id}`}
                      isWinner={id === winnerId}
                    />
                    <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                      <button
                        onClick={() => setWinner(id)}
                        className="bg-primary-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px]"
                        title="Set winner"
                      >
                        ★
                      </button>
                      <button
                        ...
                        className="bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px]"
                        title="Remove"
                      >
                        –
                      </button>
        ...
        >
          <input
            ...
            type="text"
            placeholder="Game ID"
            className="w-40 text-[11px] px-2 py-1 rounded border bg-white dark:bg-gray-800"
          />
          <button
            onClick={addNominee}
