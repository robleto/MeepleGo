    ...
    <>
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 p-1 transition-colors rounded-full hover:bg-gray-100 dark:hover:bg-gray-800"
          title={
            profile?.username ||
            profile?.full_name ||
            session.user.email ||
        ...
        >
          {profile?.avatar_url ? (
            ...
            <img
              ...
              alt="Profile"
              className="object-cover rounded-full w-7 h-7 ring-2 ring-gray-200 dark:ring-gray-700"
            />
          ) : (
          ...
          <span className="hidden text-sm font-medium text-gray-700 dark:text-gray-300 truncate sm:block max-w-24">
        ...
        {showUserMenu && (
          <>
            <div
              ref={userMenuRef}
              className="absolute right-0 z-50 hidden w-64 py-2 mt-2 text-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-xl sm:block rounded-2xl"
              role="menu"
            >
            ...
            <Link
              href="/settings"
              className="flex items-center gap-2 px-4 py-2 text-xs transition-colors hover:bg-gray-50 dark:hover:bg-gray-800"
            >
              <ListBulletIcon className="h-3.5 w-3.5 text-gray-400 dark:text-gray-500" />
              Settings
            </Link>
            <button
              onClick={onSignOut}
              className="flex items-center w-full gap-2 px-4 py-2 text-xs text-red-600 dark:text-red-400 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800"
            >
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['system', 'light', 'dark'] as const).map((m) => (
                  <button
                    key={m}
                    onClick={() => {
                      console.log('[NavigationUserMenu Desktop] Theme button clicked:', m)
                      onSetTheme(m)
                    }}
                    className={`flex items-center justify-center gap-1 px-2 py-1.5 rounded-md text-[11px] font-medium border transition ${
                      themeMode === m
                        ...
                        : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400'
                    }`}
                  >
                    {m === 'system' ? (
          ...
          <Portal>
            ...
            >
              <div
                ...
                className="fixed inset-y-0 right-0 flex flex-col w-full overflow-y-auto bg-white dark:bg-gray-900 shadow-2xl"
              ...
              >
                ...
                <div className="flex-1 py-4">
                    ...
                    {profileMenuItems.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          href={item.href}
                          onClick={() => setShowUserMenu(false)}
                          className="flex items-center gap-3 px-4 py-3 text-base font-medium rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800"
                        >
                  ...
                  {/* Settings */}
                  <div className="px-2 pt-4 mt-6 border-t border-gray-200 dark:border-gray-700">
                    <Link
                      href="/settings"
                      onClick={() => setShowUserMenu(false)}
                      className="flex items-center gap-3 px-4 py-3 text-base font-medium rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800"
                  ...
                  <div className="px-2 mt-4">
                    <div className="px-4 py-2 text-xs font-semibold tracking-wider text-gray-500 dark:text-gray-400 uppercase">
                      Theme
                    </div>
                    <div className="flex gap-2 px-4 py-2">
                      {(['system', 'light', 'dark'] as const).map((themeOption) => (
                        <button
                          key={themeOption}
                          onClick={() => {
                            console.log('[NavigationUserMenu Mobile] Theme button clicked:', themeOption)
