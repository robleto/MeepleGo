...
export default function AwardsDebugInfo({ year, awards, totalRankings = 0, qualifyingRankings = 0 }: DebugProps) {
  ...
  return (
    <div className="mt-4 p-3 bg-gray-50 rounded text-xs border">
      ...
      <div className="space-y-1">
        ...
        <div>Qualifying rankings (year={year}, played=true, rating≥7): {qualifyingRankings}</div>
        <div>Awards generated: {awards.length}</div>
        <div>Categories with nominees: {awards.filter(a => a.nominees?.length > 0).length}</div>
        {awards.length > 0 && (
