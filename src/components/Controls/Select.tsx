...
const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ className, size='md', state='default', hasLabel=false, leftIcon, placeholder, disabled, children, ...props }, ref) => {
    ...
    const iconPadding = { sm: leftIcon ? 'pl-8' : '', md: leftIcon ? 'pl-10' : '', lg: leftIcon ? 'pl-12' : '' }
    ...
    return (
      <div className='relative'>
        {leftIcon && (
          <div className={cn('absolute inset-y-0 flex items-center pointer-events-none text-gray-400 z-10', iconPosition[size])}>
            <div className={iconSize[size]}>{leftIcon}</div>
          </div>
        )}
        <select
          ref={ref}
          className={cn('w-full border rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors focus:outline-none appearance-none cursor-pointer', sizeClasses[size], iconPadding[size], 'pr-8', stateClasses[state], disabled && 'opacity-50 cursor-not-allowed bg-gray-50 dark:bg-gray-800', className)}
          disabled={disabled}
          {...props}
        >
        ...
        <div className={cn('absolute inset-y-0 right-0 flex items-center pointer-events-none text-gray-400', size==='sm'?'pr-2.5': size==='lg'?'pr-3.5':'pr-3')}>
