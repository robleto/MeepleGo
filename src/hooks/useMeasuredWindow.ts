...
export function useMeasuredWindow<T>(
  opts: UseMeasuredWindowOptions<T>
): UseMeasuredWindowResult<T> {
  const {
    items,
    enabled,
    overscan = 4,
  ...
  const avg = totalMeasured
    ? heightsRef.current.filter((h) => h > 0).reduce((a, b) => a + b, 0) /
      totalMeasured
    : averageHeight
  const totalHeight =
    heightsRef.current.length === items.length && totalMeasured === items.length
  ...
  if (heightsRef.current.length !== items.length) {
    heightsRef.current = Array.from(
      { length: items.length },
      (_, i) => heightsRef.current[i] || 0
    )
    nodesRef.current = Array.from(
      { length: items.length },
      (_, i) => nodesRef.current[i] || null
    )
  }
  ...
  const recompute = useCallback(() => {
    ...
    const range = computeWindowRange(
      heightsRef.current.map((h) => h || avg),
      scrollTop,
      clientHeight,
      overscan
    )
  ...
  const spacerTop = heightsRef.current
    .slice(0, range.start)
    .reduce((a, b) => a + (b || avg), 0)
  const visibleHeights = heightsRef.current
  ...
  const visible = items
    .slice(range.start, range.end)
    .map((item, i) => ({ item, index: range.start + i }))
