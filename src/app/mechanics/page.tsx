...
export default async function MechanicsPage() {
  ...
  if (error) {
    return (
      <PageLayout>
        <div className="py-12">
          <p className="text-red-600">Failed to load mechanics.</p>
        </div>
      </PageLayout>
    )
  }
  ...
  const mechanics: MechanicWithCount[] = (data || []).map((m) => ({
    ...
    game_count: Array.isArray((m as any).game_mechanics)
      ? (m as any).game_mechanics.length
      : 0,
  }))
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {mechanics.map((mech) => (
            <Link
              key={mech.id}
              href={`/mechanics/${mech.slug}`}
              className="group block rounded-lg border border-gray-200 bg-white p-4 hover:border-primary-400 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
              <div className="flex items-start justify-between">
                <h2 className="font-semibold text-gray-900 group-hover:text-primary-600">
                  {mech.name}
                </h2>
