          ...
          <div className="mb-8">
            ...
            {!loading && query.length >= 2 && results.length === 0 && (
              <div className="mt-3 text-xs text-gray-500">
                No matches.{' '}
                <a href="/games/add-missing" className="underline">
                  Add a missing game?
                </a>
              </div>
            )}
                ...
                {results.map((g) => {
                  ...
                  if (idx >= 0) {
                    display = (
                      <span>
                        {g.name.slice(0, idx)}
                        <span className="bg-yellow-200 text-gray-900 px-0.5 rounded-sm">
                          {g.name.slice(idx, idx + term.length)}
                        </span>
                        {g.name.slice(idx + term.length)}
                      </span>
                    )
                  }
                  return (
