...
export default async function AwardsPage({
  ...
  let personalCategoryBlocks: Array<{
    id: string
    label: string
    games: any[]
    description?: string
  ...
  if (session?.user) {
    const { data: rankingRows } = await supabase
      .from('rankings')
      .select(
        'game_id, ranking, played_it, games:game_id ( id, name, year_published, image_url, thumbnail_url, categories, mechanics, min_players, max_players, honors, playtime_minutes )'
      )
    ...
    const defs: Array<{
    ...
    }> = [
      {
        id: 'best',
        label: 'Best Overall',
        description: 'Your all‑time highest rated played games.',
        filter: () => true,
      },
      {
        id: 'strategy',
        label: 'Best Strategy',
        description: 'Depth, planning, crunchy decisions.',
        filter: (r) =>
      ...
      },
      {
        id: 'family',
        label: 'Best Family',
        description: 'Accessible & fun for mixed groups.',
        filter: (r) =>
          (r.game.categories || []).some((c: string) =>
      ...
      },
      {
        id: 'duo',
        label: 'Best Duo',
        description: 'Top two‑player experiences.',
        filter: (r) =>
      ...
      },
      {
        id: 'kids',
        label: 'Best Kids',
        description: 'Great for younger players.',
        filter: (r) =>
          (r.game.categories || []).some((c: string) =>
      ...
      },
      {
        ...
        description: 'Card‑driven experiences.',
        filter: (r) =>
          (r.game.categories || []).some((c: string) =>
      ...
      },
      {
        id: 'wargame',
        label: 'Best Wargame',
        description: 'Conflict & historical simulation.',
        filter: (r) =>
          (r.game.categories || []).some((c: string) =>
      ...
      },
      {
        id: 'trivia',
        label: 'Best Trivia',
        description: 'Quiz & fact showdowns.',
        filter: (r) =>
          (r.game.categories || []).some((c: string) =>
      ...
      },
      {
        ...
        description: 'DIY / print & play titles.',
        filter: (r) =>
          (r.game.categories || []).some((c: string) =>
      ...
      },
      {
        ...
        description: 'Work together vs the game.',
        filter: (r) =>
      ...
      },
      {
        ...
        description: 'Progress via evolving decks.',
        filter: (r) =>
          (r.game.mechanics || []).some((m: string) =>
      ...
      },
      {
        ...
        description: 'Great single‑player experience.',
        filter: (r) =>
          (r.game.mechanics || []).some((m: string) =>
      ...
      },
      {
        id: 'abstract',
        label: 'Best Abstract',
        description: 'Pure mechanisms & elegance.',
        filter: (r) =>
      ...
      },
      {
        id: 'thematic',
        label: 'Best Thematic',
        description: 'Story & immersion focused.',
        filter: (r) =>
          (r.game.categories || []).some((c: string) =>
      ...
      },
      {
        ...
        description: 'Quick to learn & play (<45m).',
        filter: (r) => (r.game.playtime_minutes ?? 999) <= 45,
      },
      {
        id: 'medium',
        label: 'Best Medium Weight',
        description: 'Mid-weight strategy (~46-90m).',
        filter: (r) =>
      ...
      },
      {
        ...
        description: 'Long form or epic sessions.',
        filter: (r) => (r.game.playtime_minutes ?? 0) > 100,
      },
    ]

    personalCategoryBlocks = defs
      .map((def) => ({
        id: def.id,
        label: def.label,
    ...
    <PageLayout>
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {serverLoggedIn ? (
          <div className="mt-16">
            <div className="mb-5">
              <h2 className="text-md font-semibold tracking-wide text-gray-400 uppercase">
                Personal Awards
              </h2>
            </div>
            <div className="space-y-12">
              {personalCategoryBlocks.length === 0 && (
                <p className="text-xs text-gray-500 text-center">
                  Rate and rank games you have played to start generating your
                  personal awards.
                </p>
              )}
              ...
              {personalCategoryBlocks.length > 0 && (
                <div className="pt-2 text-center text-[11px] text-gray-400">
                  Auto Awards are generated from your played & rated games using
                  relaxed category and mechanic matches; rate more games to
