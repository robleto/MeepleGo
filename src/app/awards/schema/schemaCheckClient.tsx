...
export default function SchemaCheckClient() {
  ...
  if (loading)
    return <div className="mt-6 text-sm text-gray-500">Checking…</div>
  ...
  return (
    <div className="mt-6">
      <div className="flex items-center gap-3 mb-2">
        <span
          className={`text-xs px-2 py-1 rounded ${data.needsAction ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}
        >
          {data.needsAction ? 'Needs Action' : 'Schema OK'}
        </span>
        <button onClick={run} className="text-xs underline">
          Re-run
        </button>
      </div>
