...
async function getGameBySlug(slug: string): Promise<Game | null> {
  ...
  if (!/^\d+$/.test(slug) && !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(slug)) {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = game.description || `Learn about ${game.name}, a board game${game.year_published ? ` published in ${game.year_published}` : ''}${game.designer ? ` by ${game.designer}` : ''}.`
  
  const playerInfo = game.min_players && game.max_players 
  ...
  const fullDescription = gameInfo 
...
export default async function GamePage({ params }: PageProps) {
  ...
  } else if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(resolvedParams.slug)) {
  ...
  const gameData = {
    ...
    "numberOfPlayers": game.min_players && game.max_players ? {
    ...
    "aggregateRating": game.rating && game.num_ratings ? {
