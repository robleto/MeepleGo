...
export async function GET() {
  ...
  return NextResponse.json({
    env: {
      supabaseUrlHost: (()=>{ try { return new URL(process.env.NEXT_PUBLIC_SUPABASE_URL||'').host } catch { return null } })(),
      anonKeyPrefix: (process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||'').slice(0,16),
    ...
    cookieNames: ((): string[] => {
      try { return Array.from({ length: 0 }) } catch { return [] }
    })(),
...
export async function POST(req: NextRequest) {
  ...
  if (contentType.includes('application/json')) {
    ...
    if (typeof body === 'string' && body.startsWith('{')) {
      ...
      try { body = Object.fromEntries(new URLSearchParams(body).entries()) } catch {}
    }
  }
  const email = body.email?.trim()
  const password = body.password
  if (!email || !password) return NextResponse.json({ error: 'Email & password required', receivedKeys: Object.keys(body) }, { status: 400 })
  const supabase = await getSupabaseServerClient()
  ...
  return NextResponse.json({
    ...
    durationMs: dur,
    troubleshooting: !error ? undefined : [
