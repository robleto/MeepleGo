...
export async function GET(req: NextRequest) {
  try {
    ...
    while (true) {
      ...
      if (daySet.has(key)) { currentStreak++; cursor.setDate(cursor.getDate()-1); } else break
    }
    ...
    if (allDaysSorted.length) {
      ...
      for (let i=1;i<allDaysSorted.length;i++) {
        ...
        if (diff === 1) { run++; } else { if (run>longestStreak) longestStreak=run; run=1 }
      }
    ...
    const dailyAvg: { date:string; avg:number }[] = allDaysSorted.map(d=> {
