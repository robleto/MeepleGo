...
export async function POST(
  ...
  try {
    ...
    const { data: rows, error } = await supabase
      ...
      .select(
        'game_id, rating:ranking, played_it, updated_at, games:game_id ( id, name, year_published, categories, mechanics, min_players, max_players )'
      )
      .eq('user_id', session.user.id)

    if (error)
      return NextResponse.json(
        { error: error.message, stage: 'fetch_rankings' },
        { status: 500 }
      )
    ...
    if (existingErr)
      return NextResponse.json(
        { error: existingErr.message, stage: 'fetch_existing_awards' },
        { status: 500 }
      )

    const manualSet = new Set(
      (existingRows || [])
        .filter((r) => (r as any).manual_override)
        .map((r) => r.category)
    )
    ...
    if (staleOnly) {
      ...
      if (staleErr)
        return NextResponse.json(
          { error: staleErr.message, stage: 'fetch_stale_awards' },
          { status: 500 }
        )
    ...
    for (const d of derived) {
      ...
      if (upErr)
        return NextResponse.json(
          { error: upErr.message, stage: 'upsert_award', category: d.category },
          { status: 500 }
        )
    }
    return NextResponse.json({
      ...
      year: yearNum,
      awards: derived,
      skipped_manual: Array.from(manualSet),
      staleOnly,
    })
  } catch (e: any) {
