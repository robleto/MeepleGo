...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ year: string; category: string }> }) {
  ...
  if (!session) {
    ...
    if (authHeader.startsWith('Bearer ')) {
      ...
      const bearerClient = createClient<Database>(supabaseUrl, supabaseAnon, { global: { headers: { Authorization: `Bearer ${token}` } } })
