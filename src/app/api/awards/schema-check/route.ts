...
export async function GET() {
  ...
  const { data: { session } } = await supabase.auth.getSession().catch(()=>({ data:{ session:null }}))
    ...
    // columns
    let cols: any = null; let colErr: any = null
    try {
      const resp: any = await (supabase as any).rpc('introspect_awards_columns')
      cols = resp.data; colErr = resp.error
    } catch {}
    ...
    const expected = [
      'id','user_id','year','category','nominees','winner_id','created_at','updated_at','manual_override','threshold_used','refreshed_at','stale'
    ]
    const missing = expected.filter(c => !columnNames.includes(c))
    const legacy = ['profile_id','nominee_ids'].filter(c => columnNames.includes(c))

    // Policies: we cannot query pg_policies via anon key unless service role or elevated; attempt and tolerate failure.
    let policies: any[] = []
    try {
      const { data: pol } = await (supabase as any).from('pg_policies').select('*').eq('tablename','awards')
      policies = pol || []
    } catch {}

    const userIdPolicyPresent = policies.some(p => /user_id\s*=\s*auth\.uid\(\)/i.test(JSON.stringify(p)))
    const profileIdPolicyPresent = policies.some(p => /profile_id\s*=\s*auth\.uid\(\)/i.test(JSON.stringify(p)))
    ...
    return NextResponse.json({
      ...
      hints: {
        ...
        message: authLikelyMissing ? 'No server session detected; RLS returned 0 rows so columns could not be inferred.' : (emptyDetected ? 'Awards table has no rows yet; generate awards to confirm columns.' : null)
      },
      needsAction: missing.length > 0 || legacy.length > 0 || profileIdPolicyPresent || !userIdPolicyPresent
    })
  } catch (e:any) {
