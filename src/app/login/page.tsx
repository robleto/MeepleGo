...
function LoginForm() {
  ...
  const [status, setStatus] = useState<
    'idle' | 'auth-check' | 'submitting' | 'redirecting' | 'magic-sent'
  >('idle')
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (/invalid login credentials/i.test(error.message)) {
        ...
        setInfo(
          'Tips: 1) Make sure you used the most recent reset email link. 2) Email comparison is case-insensitive but password is case-sensitive. 3) If you just changed the password in another browser/profile, hard refresh here. 4) Ensure this environment matches where you reset (dev vs prod project).'
        )
      } else if (/email not confirmed/i.test(error.message)) {
        setError('Email not confirmed.')
        setInfo(
          'Check your inbox for the confirmation email, then try again after confirming. You can also resend it below.'
        )
      } else {
    ...
    if (!data.session) {
      setInfo(
        'Sign-in returned without a session. Try clearing cookies/localStorage for this site and retry.'
      )
      setStatus('idle')
      return
    }
  ...
  const resendConfirmation = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setInfo(
          'If the account exists and is unconfirmed, a new confirmation email was sent.'
        )
      }
    } finally {
      ...
      <form onSubmit={onSubmit} className="space-y-5" noValidate>
        ...
        <div aria-live="polite" className="space-y-2">
          ...
          {status === 'submitting' && !error && (
            <p className="text-[11px] text-primary-600 flex items-center gap-2">
              <span className="w-3 h-3 border-2 border-primary-400 border-t-transparent rounded-full animate-spin" />{' '}
              Authenticating…
            </p>
          )}
          {status === 'redirecting' && (
            <p className="text-[11px] text-primary-600">Redirecting…</p>
          )}
        </div>
        <button
          type="submit"
        ...
        <div className="flex items-center justify-between text-xs pt-1">
          <button
            ...
            disabled={!email || magicSending || magicSent}
            className="underline disabled:opacity-40"
          >
            {magicSent
              ? 'Magic link sent'
