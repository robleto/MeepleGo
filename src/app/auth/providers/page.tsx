...
export default function AuthProvidersTestPage() {
  ...
  const testSignup = async () => {
    ...
    setStatus(
      error
        ? `Error: ${error.message}`
        : 'Sent. Check your inbox for the confirmation email.'
    )
  }
  ...
  const testLogin = async () => {
    ...
    setStatus(
      error
        ? `Error: ${error.message}`
        : 'Logged in (if credentials are valid).'
    )
  }
  ...
  const testReset = async () => {
    ...
    setStatus(
      error
        ? `Error: ${error.message}`
        : 'Sent. Check your inbox for the reset email.'
    )
  }
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <p className="text-gray-600 mb-6">
        Quickly test Email and OAuth providers configured in Supabase.
      </p>
      ...
      <div className="space-y-6">
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          {session && (
            <button
              onClick={signOut}
              className="mt-3 inline-flex items-center px-3 py-2 rounded-md bg-gray-900 text-white hover:bg-gray-800"
            >
              Sign out
            </button>
          )}
