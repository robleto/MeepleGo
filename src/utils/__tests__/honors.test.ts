...
describe('inferHonorCategory', () => {
  ...
  it('detects truncated slug -winn as Winner', () => {
    expect(
      inferHonorCategory({ slug: '2020-some-award-best-family-game-winn' })
    ).toBe('Winner')
  })
  it('detects truncated slug -nomin as Nominee', () => {
    expect(
      inferHonorCategory({ slug: '2020-some-award-best-family-game-nomin' })
    ).toBe('Nominee')
  })
  ...
  it('interprets recommended as Special', () => {
    expect(inferHonorCategory({ title: 'Spiel des Jahres Recommended' })).toBe(
      'Special'
    )
  })
})
