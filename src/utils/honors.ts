 ...
 */
export function inferHonorCategory(
  h: RawLikeHonor,
  context?: { gameCount?: number }
): HonorCategory {
  ...
  const corpus =
    `${h.slug || ''} ${h.title || ''} ${h.name || ''}`.toLowerCase()
  if (
    /(winner|winne|winn)(\b|$)/.test(corpus) ||
    /-winn$/.test(corpus) ||
    /-winne$/.test(corpus)
  )
    return 'Winner'
  if (
    /(nominee|nomin|nom)(\b|$)/.test(corpus) ||
    /-nomin$/.test(corpus) ||
    /-nom$/.test(corpus)
  )
    return 'Nominee'
  ...
  if (h.award_type && /golden geek/i.test(h.award_type)) {
    const hasExplicit = /(winner|winne|winn|nominee|nomin|nom)(\b|$)/.test(
      corpus
    )
  ...
  if (corpus.includes('recommended') || corpus.includes('special'))
    return 'Special'
...
export function normalizeHonorCategories<T extends RawLikeHonor>(
  honors: T[]
): T[] {
  return honors.map((h) => {
    ...
    if (h.category !== inferred) {
      return {
        ...h,
        _originalCategory: h.category,
        category: inferred,
      } as T & { _originalCategory?: HonorCategory }
    }
    return h
  })
}
