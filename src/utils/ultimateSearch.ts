 ...
 */
function createSearchVariations(
  query: string,
  gameName: string
): Array<{ query: string; name: string }> {
  const variations = []

  // Original
  variations.push({
    query: query.toLowerCase().trim(),
    name: gameName.toLowerCase().trim(),
  })
  ...
  for (const { query: q, name: n } of variations) {
      ...
      const matchedWords = queryWords.filter((queryWord) =>
        nameWords.some(
          (nameWord) =>
            ...
            (queryWord.length > 2 &&
              nameWord.length > 2 &&
              (nameWord.startsWith(queryWord.slice(0, 3)) ||
                queryWord.startsWith(nameWord.slice(0, 3))))
        )
      )
      ...
      if (matchedWords.length === queryWords.length) {
        ...
        if (score > bestMatch.score) {
          bestMatch = {
            isMatch: true,
            score,
            matchType: 'partial-words-variation',
          }
        }
      }
    }
    ...
    if (queryWords.length === 1) {
      ...
      if (singleWord && singleWord.length > 2) {
        const foundInName = nameWords.some(
          (nameWord) =>
            nameWord.includes(singleWord) ||
            singleWord.includes(nameWord) ||
            (nameWord.length > 2 &&
              singleWord.length > 2 &&
              (nameWord.startsWith(singleWord.slice(0, 3)) ||
                singleWord.startsWith(nameWord.slice(0, 3))))
        )
        ...
        if (foundInName) {
          ...
          if (score > bestMatch.score) {
            bestMatch = {
              isMatch: true,
              score,
              matchType: 'single-word-variation',
            }
          }
        }
      }
...
export class UltimateGameSearch {
  ...
  searchWithScores(
    query: string,
    limit: number = 50
  ): Array<{ game: GameWithRanking; score: number; matchType: string }> {
    if (!query.trim()) return []

    const results = new Map<
      string,
      { game: GameWithRanking; score: number; matchType: string }
    >()
 ...
 */
export function ultimateSearchGames(
 ...
 */
export function ultimateSearchWithDebug(
  games: GameWithRanking[],
  query: string,
  limit: number = 50
) {
