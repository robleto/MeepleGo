    ...
    for (let lineIndex = 1; lineIndex < lines.length; lineIndex++) {
      ...
      try {
        ...
        if (values.length !== finalHeaders.length) {
          errors.push(
            `Line ${lineIndex + 1}: Expected ${finalHeaders.length} columns, found ${values.length}`
          )
        }
        ...
        errors.push(
          `Line ${lineIndex + 1}: Failed to parse - ${error instanceof Error ? error.message : 'Unknown error'}`
        )
      }
    }
...
export function parseCSVBoolean(value: string): boolean | null {
  ...
  // True values
  if (
    ['true', 't', 'yes', 'y', '1', 'on', 'owned', 'played'].includes(normalized)
  ) {
    return true
  }
