...
function parseXMLToGame(xmlText: string): any[] {
  ...
  for (const itemMatch of itemMatches) {
    try {
      ...
      const categoryMatches = itemMatch.match(
      ...
      const mechanicMatches = itemMatch.match(
      ...
      if (bggIdMatch && nameMatch) {
        ...
        console.log(
          `   Players: ${game.min_players}-${game.max_players}, Time: ${game.playtime_minutes}min`
        )
        console.log(`   Rating: ${game.rating}, Rank: ${game.rank}`)
        console.log(
          `   Categories: [${game.categories.slice(0, 3).join(', ')}${game.categories.length > 3 ? '...' : ''}] (${game.categories.length})`
        )
        console.log(
          `   Mechanics: [${game.mechanics.slice(0, 3).join(', ')}${game.mechanics.length > 3 ? '...' : ''}] (${game.mechanics.length})`
        )
        console.log(
          `   Images: ${game.image_url ? '✅' : '❌'} Description: ${game.description ? '✅' : '❌'}`
        )
        // TODO: Add summary logging when column is added to DB
        // console.log(`   Summary: "${summary}"`);
      } else {
        console.warn(
          `⚠️ Could not parse game from XML segment (missing ID or name)`
        )
      }
    } catch (error) {
  ...
  while (retries < BGG_RATE_LIMIT.MAX_RETRIES) {
    try {
      ...
      if (response.status === 429) {
        console.warn(
          `⏳ Rate limited, retrying in ${BGG_RATE_LIMIT.ERROR_DELAY}ms...`
        )
...
async function fetchAndStoreGames(supabase: any, ids: number[]) {
  ...
  console.log(
    `🔄 Processing ${ids.length} games in ${totalBatches} batches of ${BGG_RATE_LIMIT.BATCH_SIZE}`
  )
  ...
  for (let i = 0; i < ids.length; i += BGG_RATE_LIMIT.BATCH_SIZE) {
    ...
    console.log(
      `📦 Batch ${batchNumber}/${totalBatches}: Fetching games ${batch.join(',')}`
    )
    ...
    while (retries < BGG_RATE_LIMIT.MAX_RETRIES) {
      try {
        ...
        if (!res.ok) {
          console.warn(
            `⚠️ Failed to fetch batch ${batchNumber}: HTTP ${res.status}`
          )
          errorCount++
          break
        }
        ...
        for (const game of games) {
          try {
            const { data: upserted, error } = await supabase
              ...
              .select('id, categories, mechanics, publisher')
              .single()

            if (error) {
              console.error(
                `❌ Error saving game ${game.bgg_id}:`,
                error.message
              )
              errorCount++
            } else {
              ...
              await ensureCategoryLinks(
                supabase,
                gameId,
                (upserted.categories || []) as string[]
              )
              await ensureMechanicLinks(
                supabase,
                gameId,
                (upserted.mechanics || []) as string[]
              )
              await ensurePublisherLink(
                supabase,
                gameId,
                upserted.publisher as string | null
              )
            }
          } catch (dbError) {
        ...
        console.error(
          `❌ Network error on batch ${batchNumber}, attempt ${retries + 1}:`,
          err
        )
        retries++
        if (retries < BGG_RATE_LIMIT.MAX_RETRIES) {
    ...
    if (i + BGG_RATE_LIMIT.BATCH_SIZE < ids.length) {
      console.log(
        `⏳ Waiting ${BGG_RATE_LIMIT.MIN_DELAY}ms before next batch...`
      )
  ...
  console.log(
    `🎉 Import completed: ${storedCount} games stored, ${errorCount} errors`
  )
  ...
  for (const name of categoryNames) {
    try {
      ...
      const { data: catRows, error: catErr } = await supabase
        ...
        .single()
      if (catErr) {
        console.warn('Category upsert failed', name, catErr.message)
        continue
      }
      ...
      const { error: linkErr } = await supabase
        ...
        .upsert(
          { game_id: gameId, category_id },
          { onConflict: 'game_id,category_id' }
        )
      if (linkErr)
        console.warn('game_category link failed', name, linkErr.message)
    } catch (e) {
      console.warn('ensureCategoryLinks exception', e)
    }
  }
}

async function ensureMechanicLinks(
  supabase: any,
  gameId: string,
  ...
  for (const name of mechanicNames) {
    try {
      ...
      const { data: mechRows, error: mechErr } = await supabase
        ...
        .single()
      if (mechErr) {
        console.warn('Mechanic upsert failed', name, mechErr.message)
        continue
      }
      ...
      const { error: linkErr } = await supabase
        .from('game_mechanics')
        .upsert(
          { game_id: gameId, mechanic_id },
          { onConflict: 'game_id,mechanic_id' }
        )
      if (linkErr)
        console.warn('game_mechanic link failed', name, linkErr.message)
    } catch (e) {
      console.warn('ensureMechanicLinks exception', e)
    }
  }
}

async function ensurePublisherLink(
  supabase: any,
  gameId: string,
  ...
  try {
    ...
    const { data: pubRow, error: pubErr } = await supabase
      ...
      .single()
    if (pubErr) {
      console.warn('Publisher upsert failed', publisherName, pubErr.message)
      return
    }
    ...
    const { error: linkErr } = await supabase
      ...
      .upsert(
        { game_id: gameId, publisher_id },
        { onConflict: 'game_id,publisher_id' }
      )
    if (linkErr)
      console.warn('game_publisher link failed', publisherName, linkErr.message)
  } catch (e) {
    console.warn('ensurePublisherLink exception', e)
  }
}
