...
async function fixJustOne() {
  ...
  const justOneHonors = honors.filter(
    (h) => h.boardgames && h.boardgames.some((g) => g.bggId === 254640)
  )
  ...
  for (const entry of justOneHonors) {
    const awardType = entry.awardSet
      ? entry.awardSet.replace(/^\d{4}\s+/, '').trim()
      : null
    ...
    else if (entry.slug && entry.slug.includes('recommended'))
      category = 'Special'
