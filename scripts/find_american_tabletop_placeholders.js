...
async function run(){
  ...
  const list=[...rows.entries()].filter(([id,name])=>/award|awards|winner|nominee|finalist|recommended|best/i.test(name)).map(([bgg_id,name])=>({bgg_id,name})).sort((a,b)=>a.bgg_id-b.bgg_id);
  if(!list.length){console.log('No American Tabletop placeholders found.');return;}
