...
const BACKUP_FILE = path.join(__dirname, '../data/honors/enhanced-honors-complete.backup-categories.json');
...
function extractCategory(honor) {
  ...
  const statusWords = ['winner', 'nominee', 'recommended', 'finalist', 'honorable mention', 'runner-up'];
  ...
  const categoryStarters = ['best', 'beautiful', 'special', 'game of', 'most', 'outstanding', 'excellence'];
...
async function processHonorsForCategories(testMode = false, testCount = 10) {
  try {
    ...
    const totalCount = testMode ? Math.min(testCount, honors.length) : honors.length;
    console.log(`📊 Processing ${totalCount} honors ${testMode ? '(TEST MODE)' : ''}`);
    ...
    Array.from(categoriesFound).slice(0, 10).forEach(category => {
...
const testCount = args.includes('--count') ? parseInt(args[args.indexOf('--count') + 1]) || 10 : 10;
