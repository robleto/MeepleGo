...
class BGGRankBasedImporter {
  ...
  async fetchBGGRankPage(pageNumber) {
    ...
    console.log(
      `📡 Fetching BGG rank page starting from rank ${(pageNumber - 1) * 100 + 1}...`
    )
  ...
  parseRankPageForGameIds(html) {
    ...
    console.log(
      `🎯 Found ${newGameIds.length} new games on rank page ${Math.floor((gameIds[0] || 1) / 100) + 1}`
    )
  ...
  parseXMLToGame(xmlText) {
    ...
    for (const itemMatch of itemMatches) {
      try {
        ...
        // Parse designers
        const designerMatches = itemMatch.match(
        ...
        // Parse artists
        const artistMatches = itemMatch.match(
          /<link[^>]+type="boardgameartist"[^>]+value="([^"]+)"/g
        )
        ...
        const playtimeMinutes = playtimeMatch
          ? parseInt(playtimeMatch[1])
          : null
        ...
        // Parse statistics
        const statisticsMatch = itemMatch.match(
  ...
  async fetchGameDetails(gameIds) {
    ...
    console.log(
      `🎮 Processing games starting from rank ${Math.floor((gameIds[0] || 2501) / 100) * 100 + 1}`
    )
  ...
  async storeGame(game) {
    try {
      const { error } = await this.supabase.from('games').insert(game)
  ...
  async importGamesByRankRange(startRank, endRank) {
    ...
    console.log(
      `🏆 Starting BGG rank-based import for ranks ${startRank}-${endRank}...`
    )
        ...
        for (let i = 0; i < gameIds.length; i += batchSize) {
          ...
          try {
            ...
            for (const game of games) {
              try {
                console.log(
                  `🏆 [Rank ~${(page - 1) * 100 + 1}+] Fetching game ${game.bgg_id}...`
                )
                console.log(
                ...
                if (this.processedCount % 50 === 0) {
                  ...
                  const progress =
                    ((page - startPage) / (endPage - startPage + 1)) * 100

                  console.log(
                    `\n📊 Progress: ${this.storedCount} games stored | ~${progress.toFixed(1)}% through rank range`
                  )
                  console.log(
                    `⏱️ Rate: ${rate.toFixed(1)} games/sec | Current rank range: ~${(page - 1) * 100 + 1}`
                  )
                  console.log(
                ...
                console.error(
                  `❌ Error processing game ${game.bgg_id}:`,
                  gameError.message
                )
                this.errorCount++
              }
    ...
    if (rankData?.length > 0) {
      ...
      console.log(
        `🥇 Coverage: rank ${rankData[0].rank} to ${rankData[rankData.length - 1].rank}`
      )
    }
    ...
    if (sampleGames?.length > 0) {
      ...
      sampleGames.forEach((game, index) => {
        console.log(
          `  ${index + 1}. ${game.name} (${game.year_published || 'Unknown'}) - Rank: ${game.rank} | Rating: ${game.rating || 'N/A'}`
        )
        if (game.categories?.length > 0) {
    ...
    console.log(
      `🏆 Users can explore games across the full spectrum of board game rankings!`
    )
  }
}
