...
function parseArgs() {
  ...
  for (const a of process.argv.slice(2)) {
    ...
    else if (a.startsWith('--page-size=')) opts.pageSize = Math.min(500, Math.max(20, parseInt(a.split('=')[1],10)))
    else if (a.startsWith('--throttle-ms=')) opts.throttleMs = Math.max(0, parseInt(a.split('=')[1],10))
  }
  return opts
}
...
function loadState(file, explicitResume) {
  if (explicitResume != null) return { afterBggId: explicitResume, processed: 0, updated: 0 }
  try {
    if (!fs.existsSync(file)) return { afterBggId: null, processed: 0, updated: 0 }
    return JSON.parse(fs.readFileSync(file,'utf8'))
  } catch { return { afterBggId: null, processed: 0, updated: 0 } }
}
function saveState(file, state) { try { ensureDir(file); fs.writeFileSync(file, JSON.stringify({...state, savedAt:new Date().toISOString()}, null, 2)) } catch {} }
...
function needsExtended(row) {
  ...
  return emptyArr(row.artists) || !row.bgg_type || emptyArr(row.rank_families) || emptyArr(row.integrates_with_ids) || emptyArr(row.expansion_ids) || (!row.parent_bgg_id && row.bgg_type === 'boardgameexpansion')
}

async function fetchBatch(opts, afterBggId) {
  let query = supabase.from('games')
    .select('id,bgg_id,name,artists,bgg_type,rank_families,integrates_with_ids,expansion_ids,parent_bgg_id', { count: 'none' })
    .not('bgg_id','is', null)
...
function parseExtended(xmlText, parser) {
  try {
    ...
    const decode = (v)=>v
    result.artists = links.filter(l=>l['@_type']==='boardgameartist').map(l=>decode(l['@_value'])).filter(Boolean)
    result.integrates_with_ids = links.filter(l=>l['@_type']==='boardgameintegration' && l['@_id']).map(l=>Number(l['@_id'])).filter(n=>Number.isFinite(n))
    let parent = null
    const expSet = new Set()
    links.filter(l=>l['@_type']==='boardgameexpansion' && l['@_id']).forEach(l=>{
      const idNum = Number(l['@_id']); if (!Number.isFinite(idNum)) return
      if (l['@_inbound']==='true') { if (!parent) parent = idNum } else expSet.add(idNum)
    })
    ...
    result.rank_families = rankArray.filter(r=>r['@_type']==='family' && r['@_name']).map(r=>String(r['@_name']).toLowerCase())
    return result
  } catch (e) {
    return null
  }
...
async function updateRow(row, ext, dryRun) {
  ...
  if (empty(row.artists) && ext.artists && ext.artists.length) patch.artists = ext.artists
  if (!row.bgg_type && ext.bgg_type) patch.bgg_type = ext.bgg_type
  if (empty(row.rank_families) && ext.rank_families && ext.rank_families.length) patch.rank_families = ext.rank_families
  if (empty(row.integrates_with_ids) && ext.integrates_with_ids && ext.integrates_with_ids.length) patch.integrates_with_ids = ext.integrates_with_ids
  if (empty(row.expansion_ids) && ext.expansion_ids && ext.expansion_ids.length) patch.expansion_ids = ext.expansion_ids
  if (!row.parent_bgg_id && ext.parent_bgg_id) patch.parent_bgg_id = ext.parent_bgg_id
  if (!Object.keys(patch).length) return false
  if (dryRun) return true
  const { error } = await supabase.from('games').update(patch).eq('id', row.id)
  if (error) { console.log('❌ DB update error', error.message); return false }
  return true
}
