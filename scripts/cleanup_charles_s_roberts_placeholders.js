...
;(async function main() {
  console.log(
    `Charles S Roberts cleanup starting (mode=${APPLY ? 'APPLY' : 'scan'})`
  )
  ...
  for (const g of data) {
    ...
    const onlyCSRHonors =
      (g.honors || []).length === 0 ||
      (g.honors || []).every((h) =>
        /charles s roberts/i.test(h.award_type || h.name || '')
      )
    if (
      lower.startsWith('charles s roberts best') &&
      emptyMedia &&
      (missingYear || truncated) &&
      onlyCSRHonors
    ) {
      placeholders.push(g)
    }
  ...
  if (!APPLY) {
    console.log(
      'Run with --apply to delete these placeholder game rows. Sample:'
    )
    placeholders
      .slice(0, 10)
  ...
  for (const p of placeholders) {
    const { error: delErr } = await supabase
      ...
      .delete()
      .eq('bgg_id', p.bgg_id)
    if (delErr) console.error('Delete failed', p.bgg_id, delErr.message)
    else {
      deleted++
