...
function loadAwardSets() {
  ...
  try {
    ...
    console.error('Parse honors JSON failed:', e.message)
    process.exit(1)
  }
  const sets = new Set()
  for (const h of data) {
...
async function main() {
  ...
  const candidates = Array.from(candidateMap.entries()).map(([bgg_id, v]) => ({
    bgg_id,
    name: v.name,
    prefix: v.prefix,
  }))
  ...
  for (let i = 0; i < ids.length; i += chunkSize) {
    ...
    console.log(
      'Running hygiene for chunk',
      i / chunkSize + 1,
      '/',
      Math.ceil(ids.length / chunkSize),
      'size',
      chunk.length
    )
...
function runHygiene(idChunk) {
  return new Promise((resolve, reject) => {
    const cmd = 'node'
    const args = [
      'scripts/fix_all_game_names_and_metadata.js',
      '--ids',
      idChunk.join(','),
      '--apply',
    ]
    ...
    child.on('exit', (code) => {
      if (code !== 0)
        return reject(new Error('Hygiene script exited with code ' + code))
      resolve()
    })
  })
}
