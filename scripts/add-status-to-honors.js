...
const INPUT_FILE = path.join(__dirname, '../data/honors/enhanced-honors-complete.json');
const OUTPUT_FILE = path.join(__dirname, '../data/honors/enhanced-honors-complete.json');
const BACKUP_FILE = path.join(__dirname, '../data/honors/enhanced-honors-complete.backup.json');
...
function determineStatus(honor) {
  ...
  const winnerPatterns = [
    'winner', 'best', 'award', 'champion', 'first place', '1st place',
    'grand prize', 'gold', 'spiel des jahres', 'game of the year',
    'mensa select', 'origins award', 'diana jones award'
  ];
  
  // Check for nominee indicators
  const nomineePatterns = [
    'nominee', 'nomination', 'shortlist', 'finalist', 'recommended',
    'honorable mention', 'runner-up', 'second place', '2nd place',
    'third place', '3rd place', 'silver', 'bronze'
  ];
  ...
  if (textToCheck.includes('recommendation') || textToCheck.includes('special')) {
    return 'Recognition';
  }
...
async function processHonors() {
  try {
    ...
    for (let i = 0; i < honors.length; i++) {
      ...
      if (!hadStatus) {
        addedStatusCount++;
        console.log(`➕ [${i + 1}/${honors.length}] Added status "${honor.status}" to: ${honor.title} (${honor.year || 'No year'})`);
      } else {
        existingStatusCount++;
        if (i % 1000 === 0) { // Show progress for existing ones every 1000
          console.log(`✓ [${i + 1}/${honors.length}] Kept existing status "${honor.status}" for: ${honor.title}`);
        }
      }
      ...
      if (i % 500 === 0 && i > 0) {
        console.log(`📈 Progress: ${i}/${honors.length} (${Math.round((i/honors.length) * 100)}%)`);
      }
    }
