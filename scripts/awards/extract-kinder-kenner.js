...
const kinderIds = [
  12739, 12740, 12741, 12742, 12744, 12745, 12746, 12747, 12748, 12749, 12750,
  12751, 12752, 12753, 12754, 12755, 12756, 12758, 19364, 19365, 19366, 19367,
  19368, 19369, 19370, 19371, 19372, 19373, 19375, 19376, 19377, 19378, 19379,
  19380, 19381, 19382, 19383, 19384, 19386, 19387, 19389, 19390, 19392, 19393,
  19394, 19395, 19396, 22475, 22476, 22477, 22478, 22479, 22480, 22481, 22482,
  22483, 22484, 22485, 22486, 22487, 22488, 22489, 22490, 22491, 22506, 22507,
  22508, 22509, 22510, 22511, 22512, 22513, 22514, 22517, 22518, 22519, 22520,
  22521, 22522, 22523, 22524, 22525, 22526, 22527, 22528, 22529, 22530, 22531,
  22532, 22533, 22534, 22537, 22538, 22539, 22540, 22541, 22542, 22543, 22544,
  22545,
]

const kennerIds = [
  11078, 18371, 18372, 18504, 18521, 22570, 22573, 22854, 25144, 25147, 25490,
  27376, 27377, 27749, 35994, 35995, 36966, 41953, 41956, 42730, 48401, 48404,
  49381, 56348, 56968, 62466, 62468, 62472, 63051, 70795, 70798, 71452, 75832,
  75835, 76498, 80326, 80329, 81210, 104461, 104464, 106854, 111383, 111386,
  112339,
]
...
async function getHonorDetails(ids, awardName) {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    try {
      ...
      const titleMatch = searchHtml.match(
        new RegExp(`href="/boardgamehonor/${id}/[^"]*"[^>]*>([^<]+)</a>`)
      )
...
function generateManualEntries(honors, awardName) {
  ...
  console.log(
    `// Historical ${awardName.toLowerCase()} entries (${sortedYears.length} years: ${sortedYears[0]}-${sortedYears[sortedYears.length - 1]})`
  )
  ...
  sortedYears.forEach((year) => {
    ...
    yearGroups[year].forEach((honor) => {
      console.log(
        `      { id: ${honor.id}, slug: '${honor.slug}' }, // ${honor.category}`
      )
    })
  })
}
