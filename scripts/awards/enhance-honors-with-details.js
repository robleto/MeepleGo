...
const honorsDataset = JSON.parse(
  fs.readFileSync('complete-bgg-honors-with-titles.json', 'utf8')
)
...
const args = {
  ...
  testSample: parseInt(
    process.argv.find((arg) => arg.startsWith('--test='))?.split('=')[1] || '0'
  ),
  saveInterval: parseInt(
    process.argv
      .find((arg) => arg.startsWith('--save-every='))
      ?.split('=')[1] || '100'
  ),
}
...
function parseHonorPageData(html, honor) {
  ...
  try {
    ...
    const primaryNameMatch = html.match(
      /<b>Primary Name<\/b>\s*<\/td>\s*<td>.*?<div[^>]*>\s*([^<]+)/s
    )
    if (primaryNameMatch) {
    ...
    const alternateNameMatch = html.match(
      /<b>Alternate Names<\/b>\s*<\/td>\s*<td>.*?<div[^>]*>\s*([^<]*)/s
    )
    if (alternateNameMatch && alternateNameMatch[1].trim()) {
      const names = alternateNameMatch[1]
        .split(/[,;]/)
        .map((name) => name.trim())
    ...
    const awardSetMatch = html.match(
      /<b>Award Set<\/b>\s*<\/td>\s*<td>.*?href="[^"]*"[^>]*>\s*([^<]+)/s
    )
    if (awardSetMatch) {
    ...
    const positionMatch = html.match(
      /<b>Position<\/b>\s*<\/td>\s*<td>.*?href="[^"]*"[^>]*>\s*([^<]+)/s
    )
    if (positionMatch) {
    ...
    const boardgamesMatch = html.match(
      /<b>Boardgames<\/b>\s*<\/td>\s*<td>(.*?)<\/td>/s
    )
    if (boardgamesMatch) {
      const boardgamesSection = boardgamesMatch[1]
      const gameLinks = boardgamesSection.match(
        /href="\/boardgame\/(\d+)\/[^"]*"[^>]*>\s*([^<]+)/g
      )

      if (gameLinks) {
        gameLinks.forEach((link) => {
          const gameMatch = link.match(
            /href="\/boardgame\/(\d+)\/[^"]*"[^>]*>\s*([^<]+)/
          )
          if (gameMatch) {
    ...
    if (result.awardSet) {
      result.awardSet = result.awardSet
        .replace(/&#039;/g, "'")
        .replace(/&quot;/g, '"')
        .replace(/&amp;/g, '&')
    }
...
async function processHonors() {
  ...
  if (args.testSample > 0) {
    ...
    const endIndex =
      args.limit > 0
        ? Math.min(args.startIndex + args.limit, honorsDataset.length)
        : honorsDataset.length
  ...
  for (let i = 0; i < honorsToProcess.length; i++) {
    ...
    try {
      console.log(
        `[${globalIndex + 1}/${honorsDataset.length}] Processing: ${honor.title || honor.slug}`
      )
...
function showStatistics(enhancedHonors) {
  ...
  const withAlternateNames = enhancedHonors.filter(
    (h) => h.alternateNames && h.alternateNames.length > 0
  )
  const withBoardgames = enhancedHonors.filter(
    (h) => h.boardgames && h.boardgames.length > 0
  )
  ...
  console.log(
    `With primary name: ${withPrimaryName.length} (${((withPrimaryName.length / enhancedHonors.length) * 100).toFixed(1)}%)`
  )
  console.log(
    `With alternate names: ${withAlternateNames.length} (${((withAlternateNames.length / enhancedHonors.length) * 100).toFixed(1)}%)`
  )
  console.log(
    `With boardgames: ${withBoardgames.length} (${((withBoardgames.length / enhancedHonors.length) * 100).toFixed(1)}%)`
  )
  console.log(
    `With award set: ${withAwardSet.length} (${((withAwardSet.length / enhancedHonors.length) * 100).toFixed(1)}%)`
  )
  console.log(
    `With position: ${withPosition.length} (${((withPosition.length / enhancedHonors.length) * 100).toFixed(1)}%)`
  )
  console.log(
    `With errors: ${withErrors.length} (${((withErrors.length / enhancedHonors.length) * 100).toFixed(1)}%)`
  )
...
async function run() {
  ...
  if (args.testSample === 0 && args.limit === 0 && !args.dryRun) {
    console.log(
      `🎉 FULL DATASET ENHANCED! All ${enhancedHonors.length} honors now include detailed page data.`
    )
  }
}
