...
const args = {
  ...
  category:
    process.argv.find((arg) => arg.startsWith('--category='))?.split('=')[1] ||
    'all',
}
...
function getAsDorHonors() {
  return honorsDataset.filter((honor) => {
    ...
    const isAsDor =
      title.includes("as d'or") || title.includes("tric trac d'or")
    const inYearRange =
      honor.year && honor.year >= args.since && honor.year <= args.until
...
function parseAwardType(title) {
  ...
  if (titleLower.includes("as d'or jeu de l'année enfant"))
    return "As d'Or - Jeu de l'Année Enfant"
  if (titleLower.includes("as d'or jeu de l'année expert"))
    return "As d'Or - Jeu de l'Année Expert"
  if (
    titleLower.includes("as d'or jeu de l'année initié") ||
    titleLower.includes("as d'or jeu de l'année initie")
  )
    return "As d'Or - Jeu de l'Année Initié"
  if (titleLower.includes("as d'or jeu de l'année"))
    return "As d'Or - Jeu de l'Année"
...
async function processAsDorHonors(honors) {
  ...
  for (const honor of honors) {
    ...
    try {
      ...
      if (!perYear[year][awardType][category])
        perYear[year][awardType][category] = new Set()

      games.forEach((game) =>
        perYear[year][awardType][category].add(game.bggId)
      )
  ...
  for (const [year, awardTypes] of Object.entries(perYear)) {
    ...
    for (const [awardType, categories] of Object.entries(awardTypes)) {
      ...
      for (const [category, gameIds] of Object.entries(categories)) {
        ...
        summary[year][awardType][category] = {
          ...
          try {
            ...
            if (!honorExists) {
              ...
              if (updateError) {
                ...
                console.log(
                  `Applied ${year} ${awardType} ${displayCategory} to ${gameData?.name || bggId}`
                )
    ...
    Object.entries(summary).forEach(([year, awardTypes]) => {
      ...
      Object.entries(awardTypes).forEach(([awardType, categories]) => {
        ...
        Object.entries(categories).forEach(([category, stats]) => {
          console.log(
            `    ${category}: ${stats.applied}/${stats.count} applied`
          )
        })
      })
    })
