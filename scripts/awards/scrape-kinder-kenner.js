...
function extractKinderAndKennerHonors(html) {
  ...
  for (const line of lines) {
    ...
    const kinderMatch = line.match(
      /href="\/boardgamehonor\/(\d+)\/[^"]*"[^>]*>([^<]*Spiel des Jahres Kinderspiel[^<]*)</i
    )
    const kennerMatch = line.match(
      /href="\/boardgamehonor\/(\d+)\/[^"]*"[^>]*>([^<]*Spiel des Jahres Kennerspiel[^<]*)</i
    )
...
function generateEntriesForAward(honors, awardName) {
  ...
  console.log(
    `// Historical ${awardName} entries (${sortedYears.length} years: ${sortedYears[0]}-${sortedYears[sortedYears.length - 1]})`
  )
...
async function main() {
  ...
  console.log(
    `Kinderspiel: ${honors.filter((h) => h.award === 'Kinderspiel').length}`
  )
  console.log(
    `Kennerspiel: ${honors.filter((h) => h.award === 'Kennerspiel').length}`
  )
