...
async function fetchExpectedGames(allNames) {
  ...
  for (let i = 0; i < names.length; i += MAX_CHUNK)
    chunks.push(names.slice(i, i + MAX_CHUNK))
  const collected = []
...
async function run() {
  ...
  if (!exp) {
    console.error('No expected set for 2024')
    process.exit(1)
  }
  const want = {
  ...
  for (const g of [...gamesWithHonors, ...expectedGames])
    mergedMap.set(g.bgg_id, g)
  ...
  for (const g of games) {
    ...
    const filtered = honors.filter((h) => {
      ...
      if (!set.has(normalizeName(g.name))) {
        summary.removed++
        return false
      }
      return true
