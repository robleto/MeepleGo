...
function isMultiEqualAward(type) {
  return MULTI_EQUAL_AWARDS.some(
    (a) => a.toLowerCase() === String(type || '').toLowerCase()
  )
}
...
function deriveSubcategory(honor) {
  ...
  if (awardType && pos.toLowerCase() === awardType.toLowerCase())
    return 'Overall'

  // Remove starting year / season prefix (e.g., 2014/Fall or 2014 Spring)
  pos = pos.replace(
    /^\d{4}(?:\s*\/?\s*(Spring|Summer|Fall|Autumn|Winter))?\s*/i,
    ''
  )
  ...
  if (!primary) {
    primary = [...group].sort(
      (a, b) =>
        ...
        (a.honor_id || '').localeCompare(b.honor_id || '')
    )[0]
  }
  group.forEach((h) => {
    h.primary_winner = false
  })
...
async function run() {
  ...
  for (let page = 1; page < pages; page++) {
    ...
    let batch
    try {
      batch = await fetchGamesBatch(from, to)
    } catch (e) {
  ...
  winnerGroups.forEach((entries, key) => {
    ...
    if (groupHonors.length === 1) singleWinnerGroups++
    else multiWinnerGroups++
    const awardType = groupHonors[0].award_type
    if (isMultiEqualAward(awardType)) {
      // Mark every winner as primary
      groupHonors.forEach((h) => {
        if (h.primary_winner !== true) {
          h.primary_winner = true
        }
      })
      multiEqualGroups++
    ...
    const currentPrimaries = groupHonors.filter(
      (h) => h.primary_winner === true
    )
