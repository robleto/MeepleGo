...
try {
  ...
  console.error('❌ Failed to parse dataset:', e.message)
  process.exit(1)
}
if (!Array.isArray(dataset)) {
  console.error('❌ Expected dataset array')
  process.exit(1)
}
...
function buildHonor(entry, awardType, game) {
  const result =
    deriveResultType(entry) ||
    deriveResultFromPosition(entry.position) ||
    'Special'
  return {
...
for (const entry of dataset) {
  ...
  for (const g of entry.boardgames) {
    ...
    if (
      !arr.some((h) => h.year === honor.year && h.category === honor.category)
    ) {
      arr.push(honor)
      sourceAssociations++
    }
...
;(async () => {
  ...
  for (const [bggId, newHonors] of perGame.entries()) {
    ...
    if (!rows || !rows.length) {
      ...
      if (APPLY && CREATE_MISSING) {
        ...
        if (insErr)
          console.error(`Insert fail bgg_id=${bggId}:`, insErr.message)
        else {
          gamesCreated++
          gamesProcessed++
        }
    ...
    const winnersByYear = newHonors
      .filter((h) => h.category === 'Winner')
      .map((h) => h.year)
    for (const wy of winnersByYear) {
      const hasWinner = existing.some(
        (h) =>
          h.award_type === awardTypeFilter &&
          h.year === wy &&
          h.category === 'Winner'
      )
      if (!hasWinner) {
        const datasetWinner = newHonors.find(
          (h) => h.year === wy && h.category === 'Winner'
        )
        if (datasetWinner) {
          existing.push(datasetWinner)
          toUpgrade++
          modified = true
        }
      }
    ...
    if (APPLY && modified) {
      const { error: updErr } = await supabase
        ...
        .eq('id', game.id)
      if (updErr) console.error(`Update error bgg_id=${bggId}:`, updErr.message)
      else gamesProcessed++
    }
  }
  ...
  if (missingGames.length) {
    ...
    console.log(
      'Missing game BGG IDs (first 25):',
      preview + (missingGames.length > 25 ? ' ...' : '')
    )
  }
})()
