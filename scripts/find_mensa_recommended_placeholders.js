#!/usr/bin/env node
/** Finder for Mensa Recommended (Mensas Select / Mind Games) placeholder game names. */
...
async function run(){
  const patterns=['% Mensa Recommended%','Mensa Recommended%','% Mensa Select%','Mensa Select%','% Mensa Mind Games%','Mensa Mind Games%'];
  const rows=new Map();
  ...
  const list=[...rows.entries()].filter(([id,name])=>/winner|select|selected|recommended|award|mind games/i.test(name)).map(([bgg_id,name])=>({bgg_id,name})).sort((a,b)=>a.bgg_id-b.bgg_id);
  if(!list.length){console.log('No Mensa placeholders found.');return;}
