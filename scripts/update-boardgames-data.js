...
async function updateBoardgamesData() {
  try {
    ...
    // Read the JSON file
    const jsonPath = path.join(__dirname, '..', 'data', 'honors', 'enhanced-honors-complete.json');
      ...
      for (const honor of batch) {
        try {
          ...
          if (data && data.length > 0) {
            updatedCount++;
            console.log(`✅ [${processedCount}/${honors.length}] Updated: ${honor.title} (${honor.id}) with ${honor.boardgames.length} games`);
          } else {
            console.log(`🔍 [${processedCount}/${honors.length}] Not found: ${honor.title} (${honor.id})`);
          }
