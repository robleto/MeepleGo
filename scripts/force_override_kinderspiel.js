...
async function fetchBGG(id) {
  ...
  if (description) {
    ...
    summary = end > -1 ? clean.slice(0, end + 1) : clean.slice(0, 240)
...
async function run() {
  console.log(
    'Force overriding Kinderspiel placeholders:',
    TARGET_IDS.join(',')
  )
  for (const id of TARGET_IDS) {
    try {
      ...
      const { error } = await supabase
        ...
        .update(patch)
        .eq('bgg_id', id)
      if (error) console.error('Update fail', id, error.message)
      else console.log('Updated', id, '->', fetched.name)
    } catch (e) {
