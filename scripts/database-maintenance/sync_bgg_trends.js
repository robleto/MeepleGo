...
function extractIds(html) {
  ...
  const liRegex =
    /<li[^>]*class=\"[^\"]*numbered-game-list__item[^\"]*\"[^>]*>([\s\S]*?)<\/li>/g
  let liMatch
  while ((liMatch = liRegex.exec(html)) !== null) {
    const segment = liMatch[1]
    // Try direct data attributes first
    const dataIdMatch = segment.match(
      /data-objectid=\"(\d+)\"|data-gameid=\"(\d+)\"|data-objectid='(\d+)'/
    )
    if (dataIdMatch) {
...
async function fetchPage(p, debug) {
  try {
    const res = await fetch(p.url, {
      headers: {
        'User-Agent':
          'Mozilla/5.0 (Macintosh; Intel Mac OS X 13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36 MeepleGoBot/1.0',
      },
    })
    if (!res.ok) {
      console.warn(`Warn: ${p.label} HTTP ${res.status}`)
    ...
    if (debug) {
      console.log(
        `[debug] ${p.label} html length=${text.length} ids=${Array.from(ids).slice(0, 20).join(',')}`
      )
      if (ids.size === 0) {
...
async function existingIds(bggIds) {
  ...
  for (let i = 0; i < bggIds.length; i += CHUNK) {
    ...
    data?.forEach((r) => {
      if (r.bgg_id) found.add(r.bgg_id)
    })
  }
  return found
}
...
async function main() {
  ...
  const pageResults = await Promise.all(
    PAGES.map((p) => fetchPage(p, opts.debug))
  )
