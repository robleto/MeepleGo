...
async function main() {
  ...
  if (!file) {
    console.error('Pass migration file path.')
    process.exit(1)
  }
  ...
  for (const stmt of statements) {
    ...
    console.log(
      'Please run this statement manually (no direct SQL exec available via client):\n',
      stmt
    )
  }
  console.log(
    'NOTE: Supabase JS client cannot execute arbitrary SQL directly without a defined RPC. Run migration via psql or Supabase SQL editor.'
  )
}
