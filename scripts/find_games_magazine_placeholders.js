...
async function run(){
  ...
  const list=[...rows.entries()].filter(([id,name])=>/winner|award|of the year|best|honors/i.test(name)).map(([bgg_id,name])=>({bgg_id,name})).sort((a,b)=>a.bgg_id-b.bgg_id);
  if(!list.length){console.log('No Games Magazine placeholders found.');return;}
