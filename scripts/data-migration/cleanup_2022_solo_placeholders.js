...
const CORRUPTED_PLACEHOLDERS = [
  57141, 105023, 142585, 142889, 159692, 164874, 206156, 324759,
]
...
async function run() {
  console.log(
    `🧹 2022 Solo placeholder cleanup (mode: ${APPLY ? 'DELETE' : 'DRY-RUN'})`
  )
  ...
  for (const game of placeholders || []) {
    const isCorrupted =
      game.name.toLowerCase().includes('golden geek') &&
    ...
    const canRestore =
      ...
      !canonicalName.toLowerCase().includes('golden geek')

    console.log(
      `${game.bgg_id}: "${game.name}" - ${isCorrupted ? 'PLACEHOLDER' : 'REAL GAME'} ${canRestore ? `(can restore to "${canonicalName}")` : ''}`
    )
  ...
  if (APPLY) {
    ...
    for (const restore of toRestore) {
      ...
      if (updateError) {
        console.error(
          `❌ Failed to restore ${restore.bgg_id}:`,
          updateError.message
        )
      } else {
  ...
  for (const game of soloGames || []) {
    const soloHonors = (game.honors || []).filter(
      (h) =>
        h.year === 2022 && (h.honor_id === '79666' || h.honor_id === '80069') // nominee and winner IDs
    )
    ...
    if (soloHonors.length > 0) {
      ...
      const updatedHonors = game.honors.map((honor) => {
        if (honor.honor_id === '79666' && honor.category !== 'Nominee') {
          console.log(
            `  📝 ${game.bgg_id} (${game.name}): ${honor.category} → Nominee`
          )
          needsUpdate = true
          categoryUpdates++
        ...
        if (honor.honor_id === '80069' && honor.category !== 'Winner') {
          console.log(
            `  🏆 ${game.bgg_id} (${game.name}): ${honor.category} → Winner`
          )
          needsUpdate = true
          categoryUpdates++
      ...
      if (needsUpdate && APPLY) {
        ...
        if (updateError) {
          console.error(
            `❌ Failed to update categories for ${game.bgg_id}:`,
            updateError.message
          )
        }
      }
