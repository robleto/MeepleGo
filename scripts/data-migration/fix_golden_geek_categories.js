...
function fixHonorCategories() {
  ...
  honors.forEach((honor) => {
    ...
    const isTruncatedPresentation =
      honor.title?.toLowerCase().endsWith('prese') ||
    ...
    if (isTruncatedPresentation) {
      ...
      if (gameCount === 1) {
        ...
        if (!honor.category || honor.category === 'Special') {
          ...
          updated++
          console.log(
            `✅ Fixed ${honor.id}: ${honor.title} -> Winner (${gameCount} game)`
          )
        }
      } else if (gameCount > 1) {
        ...
        if (!honor.category || honor.category === 'Special') {
          ...
          updated++
          console.log(
            `✅ Fixed ${honor.id}: ${honor.title} -> Nominee (${gameCount} games)`
          )
        }
      }
    }
    ...
    if (honor.awardSet?.includes('Golden Geek') && !honor.category) {
      if (corpus.includes('-winn') || corpus.endsWith('winn')) {
        ...
        updated++
        console.log(
          `✅ Fixed ${honor.id}: ${honor.title} -> Winner (truncated)`
        )
        ...
        updated++
        console.log(
          `✅ Fixed ${honor.id}: ${honor.title} -> Nominee (truncated)`
        )
      }
    }
  })
