...
async function fetchCandidates() {
  ...
  for (const v of variants) {
    const { data, error } = await supabase
      .from('games')
      .select(
        'id,bgg_id,name,description,image_url,year_published,min_players,max_players,playtime_minutes'
      )
      .ilike('name', v + '%')
...
async function main() {
  ...
  const ids = report
    .filter((r) => r.action === 'rename+enrich')
    .map((r) => r.bgg_id)
  if (ids.length) {
    console.log('\nSuggested hygiene command:')
    console.log(
      `node scripts/fix_all_game_names_and_metadata.js --ids ${ids.join(',')} --apply`
    )
  }
}
