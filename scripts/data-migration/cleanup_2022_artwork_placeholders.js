...
async function run() {
  console.log(
    `🧹 2022 Artwork placeholder cleanup (mode: ${APPLY ? 'DELETE' : 'DRY-RUN'})`
  )
  ...
  for (const game of placeholders || []) {
    ...
    console.log(
      `${game.bgg_id}: "${game.name}" - ${isCorrupted ? 'PLACEHOLDER' : 'REAL GAME'}`
    )
  ...
  for (const game of artworkGames || []) {
    const artworkHonors = (game.honors || []).filter(
      (h) =>
        h.year === 2022 && (h.honor_id === '79656' || h.honor_id === '80063') // nominee and winner IDs
    )
