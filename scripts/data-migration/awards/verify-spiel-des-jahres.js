...
;(async () => {
  ...
  for (const g of games) {
    ;(g.honors || [])
      .filter((h) => h.award_type === 'Spiel des Jahres')
      .forEach((h) => {
  ...
  Array.from(yearMap.keys())
    ...
    .forEach((year) => {
      ...
      if (w.length !== 1)
        issues.push({ year, type: 'winner-count', detail: w.length })
      const nomCap = year >= 1999 ? 3 : 0 // heuristic mirrored
      if (n.length > nomCap)
        issues.push({
          year,
          type: 'nominee-cap',
          detail: `${n.length} > ${nomCap}`,
        })
      if (r.length > 5)
        issues.push({ year, type: 'recommended-cap', detail: r.length })
    })
