  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (linkMatch) {
      ...
      for (let j = i; j < Math.min(i + 5, lines.length); j++) {
        if (
          lines[j].includes('Spiel des Jahres') &&
          !lines[j].includes('Kennerspiel') &&
          !lines[j].includes('Kinder')
        ) {
...
async function run() {
  console.log(
    '🔍 Systematically scraping BGG browse pages for ALL Spiel des Jahres honors...\n'
  )
  ...
  Object.keys(byYear)
    ...
    .forEach((year) => {
      ...
      honors.forEach((h) => {
        console.log(
          `          { id: ${h.id}, slug: '${h.slug}' }, // ${h.category}`
        )
      })
      console.log(`        ];`)
      console.log(
        `        for (const m of manual${year}) if (!filtered.find(f=>f.slug===m.slug)) filtered.push(m);`
      )
      console.log(`      }`)
      console.log('')
    })
  ...
  console.log('\n=== VALIDATION TESTS ===')
  const testEntries = allHonors
    .filter((h) => [2024, 2023, 2022].includes(h.year))
    .slice(0, 3)

  for (const entry of testEntries) {
    try {
      const response = await fetch(
        `https://boardgamegeek.com/boardgamehonor/${entry.id}/${entry.slug}`,
        {
      ...
      if (response.ok) {
        ...
        console.log(
          `✅ ${entry.year} ${entry.category}: ${gameIds.length} games found`
        )
      } else {
        console.log(
          `❌ ${entry.year} ${entry.category}: Failed to fetch (${response.status})`
        )
      }
