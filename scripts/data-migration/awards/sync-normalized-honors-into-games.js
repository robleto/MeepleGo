...
// ---- CLI ----
const argv = process.argv.slice(2).reduce((m, a) => {
  if (a.startsWith('--')) {
...
for (const h of raw) {
  ...
  for (const g of h.game_refs) {
    ...
    if (h.subcategory && h.subcategory !== 'Overall')
      nameParts.push(h.subcategory)
    ...
    const url = rawUrl
      ? rawUrl.startsWith('http')
        ? rawUrl
        : 'https://boardgamegeek.com' + rawUrl
      : null
    const honorObj = {
      honor_id: h.honor_id,
      name: honorName,
...
log('[sync-normalized] Build summary:', {
  distinct_games: perGame.size,
  honors_prepared: emitted,
})
if (DRY) {
...
;(async () => {
  ...
  for (let offset = 0; offset < entries.length; offset += batchSize) {
    ...
    for (const [bggId, honorsListNew] of slice) {
      ...
      const filtered = honorsListNew.filter((o) =>
        ['Winner', 'Nominee', 'Special'].includes(o.category)
      )
      ...
      updated++
      processed++
      if (!QUIET && updated % 250 === 0)
        console.log(`[sync-normalized] Updated ${updated} games so far...`)
    }
    if (!QUIET)
      console.log(
        `[sync-normalized] Batch ${offset / batchSize + 1} complete (processed ${processed})`
      )
  }
