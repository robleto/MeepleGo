...
async function run() {
  ...
  if (found.length === 0) {
    console.log(
      'No 2023 Spiel des Jahres honors found. Trying direct URL guess...'
    )
    ...
    for (const guess of guesses) {
      try {
        const response = await fetch(
          `https://boardgamegeek.com/boardgamehonor/${guess.id}/${guess.slug}`,
          {
  ...
  if (found.length > 0) {
    ...
    console.log(
      'None found. May need to search BGG manually or try different ID ranges.'
    )
  }
}
