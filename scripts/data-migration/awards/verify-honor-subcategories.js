...
const MULTI_EQUAL_AWARDS = ['Mensa Select', 'Meeples Choice Award']
function isSpecialMultiGroup(awardType) {
  return MULTI_EQUAL_AWARDS.some(
    (a) => a.toLowerCase() === String(awardType || '').toLowerCase()
  )
}
...
async function run() {
  ...
  let first
  try {
    first = await fetchPage(0, pageSize - 1)
  } catch (e) {
    console.error(e)
    process.exit(1)
  }
  ...
  const stats = {
    totalHonors: 0,
    honorsWithSubcategory: 0,
    winners: 0,
    winnersWithPrimary: 0,
  }
  ...
  groups.forEach((group) => {
    ...
    if (isSpecialMultiGroup(group.award_type)) {
      ...
      if (primaries.length === 0)
        anomalies.missingPrimary.push({
          award_type: group.award_type,
          year: group.year,
          winners: group.honors.length,
        })
    ...
    group.honors.forEach((h) => {
      if (!h.subcategory)
        anomalies.noSubcategory.push({
          award_type: group.award_type,
          year: group.year,
          honor_id: h.honor_id,
        })
  ...
  const summary = {
    ...
    subcategoryCoveragePct: (
      (stats.honorsWithSubcategory / Math.max(1, stats.totalHonors)) *
      100
    ).toFixed(2),
    winnerGroups: groups.size,
    winnerGroupsWithPrimary: stats.winnersWithPrimary,
    winnerGroupsPrimaryPct: (
      (stats.winnersWithPrimary / Math.max(1, groups.size)) *
      100
    ).toFixed(2),
    anomalies: {
  ...
  if (anomalies.missingPrimary.length)
    console.log(
      '[verify] missingPrimary sample:',
      anomalies.missingPrimary.slice(0, 10)
    )
  if (anomalies.multiplePrimary.length)
    console.log(
      '[verify] multiplePrimary sample:',
      anomalies.multiplePrimary.slice(0, 10)
    )
  if (anomalies.noSubcategory.length)
    console.log(
      '[verify] noSubcategory sample:',
      anomalies.noSubcategory.slice(0, 10)
    )
