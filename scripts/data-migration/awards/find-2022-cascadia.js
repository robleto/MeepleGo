...
async function testForCascadia(id, slug) {
  try {
    const response = await fetch(
      `https://boardgamegeek.com/boardgamehonor/${id}/${slug}`,
      {
    ...
    const hasCascadia =
      html.includes('/boardgame/295947/') || html.includes('Cascadia')
...
async function run() {
  console.log(
    'Searching for 2022 Spiel des Jahres honor pages containing Cascadia...'
  )
  ...
  for (const range of testRanges) {
    ...
    for (let offset = 0; offset <= range.range; offset++) {
      ...
      for (const cat of categories) {
        ...
        if (result) {
          console.log(
            `✅ Found ${cat} at ID ${id} (${result.gameIds} games, contains Cascadia)`
          )
          found.push(result)
        }
