...
const {
  fetchHonorBrowsePage,
  extractHonorLinks,
  fetchHonorPage,
  extractGameIdsFromHonor,
  ensureGame,
  supabase,
  searchGameByName,
} = require('./fetchers')
  ...
  for (let p = 1; p <= maxPages; p++) {
    try {
      ...
      if (/sign in/i.test(html) && !process.env.BGG_COOKIES) {
        console.log('Login wall encountered; stopping.')
        break
      }
      extractHonorLinks(html).forEach((h) => {
        if (!cfg.slugPatterns.some((sp) => h.slug.includes(sp))) return
        if (
          cfg.excludeSlugSubstrings &&
          cfg.excludeSlugSubstrings.some((ex) => h.slug.includes(ex))
        )
          return
        if (!seen.has(h.slug)) seen.set(h.slug, h)
      })
    } catch (e) {
      if (/status 404/.test(e.message)) break
    }
...
async function applyHonor(game, honor) {
  ...
  const { error } = await supabase
    .from('games')
    .update({ honors })
    .eq('bgg_id', game.bgg_id || game.bgg_id)
...
async function fetchAggregatePage(url) {
  const res = await fetch(url, {
    headers: {
      'user-agent':
        process.env.BGG_USER_AGENT || 'Mozilla/5.0 (MeepleGo Awards)',
    },
  })
...
function extractBggIdsFromTextBlock(block) {
  ...
  let m
  while ((m = regex.exec(block)) !== null) ids.add(parseInt(m[1]))
...
function sliceBlocksByYear(html, yearPattern) {
  ...
  const years = {}
  const matches = [...html.matchAll(new RegExp('(>?(19|20)\\d{2})<', 'g'))]
...
function categorizeAggregate(yearBlock, aggCfg) {
  ...
  for (const line of lines) {
    ...
    if (aggCfg.winnerMarkers && aggCfg.winnerMarkers.some((r) => r.test(line)))
      ids.forEach((id) => out.Winner.add(id))
    else if (
      aggCfg.nomineeMarkers &&
      aggCfg.nomineeMarkers.some((r) => r.test(line))
    )
      ids.forEach((id) => out.Nominee.add(id))
    else if (
      aggCfg.recommendedMarkers &&
      aggCfg.recommendedMarkers.some((r) => r.test(line))
    )
      ids.forEach((id) => out.Recommended.add(id))
    else {
...
async function run() {
  ...
  if (!args.award) {
    console.error('--award required')
    process.exit(1)
  }
  const cfg = findAward(args.award)
  if (!cfg) {
    console.error('Unknown award id')
    process.exit(1)
  }
  console.log(
    `=== IMPORT AWARD: ${cfg.display} (${cfg.id}) Years ${args.since}-${args.until} ===`
  )
  ...
  if (cfg.mode === 'aggregate') {
    ...
    const html = await fetchAggregatePage(cfg.aggregateUrl).catch((e) => {
      console.error('Aggregate fetch failed:', e.message)
      return null
    })
    if (!html) {
      console.warn(
        'No aggregate HTML fetched. Falling back to honor page discovery path.'
      )
      ...
      if (args.since <= 2025 && args.until >= 2025) {
        ...
        for (const m of manual2025)
          if (!filtered.find((f) => f.slug === m.slug)) filtered.push(m)
      }
      ...
      if (args.since <= 2023 && args.until >= 2023) {
        ...
        for (const m of manual2023)
          if (!filtered.find((f) => f.slug === m.slug)) filtered.push(m)
      }
      console.log(
        `Discovered ${filtered.length} honor page slugs in range (fallback).`
      )
      if (args.debug) {
        const yearsCovered = new Set(
          filtered.map((f) => yearFromSlug(f.slug)).filter(Boolean)
        )
        console.log(
          '[debug] Years covered (filtered):',
          Array.from(yearsCovered).sort().join(', ')
        )
      }
      for (const h of filtered) {
        const year = yearFromSlug(h.slug)
        if (!year) continue
        if (year < cfg.startYear) continue
        const { category } = cfg.normalizeCategory(h.slug)
        if (args.debug)
          console.log(
            `[debug] Fetching honor page ${h.id} slug=${h.slug} year=${year} category=${category}`
          )
        ...
        if (args.debug)
          console.log(
            `[debug] Extracted ${gameIds.length} game ids for ${h.slug}`
          )
        perYear[year] = perYear[year] || {}
        perYear[year][category] = perYear[year][category] || new Set()
      ...
      Object.entries(yearBlocks).forEach(([year, block]) => {
        ...
        if (y < args.since || y > args.until) return
        if (y < cfg.startYear) return
        const cats = categorizeAggregate(block, cfg.aggregate)
        perYear[y] = perYear[y] || {}
        Object.entries(cats).forEach(([cat, set]) => {
          if (set.size) perYear[y][cat] = perYear[y][cat] || new Set()
          set.forEach((v) => perYear[y][cat].add(v))
        })
      })
    }
  } else {
  ...
  for (const [yearStr, catMap] of Object.entries(perYear)) {
    ...
    if (expectedYear) {
      ...
      if (args.strict && (missing.length || unexpected.length))
  ...
  if (args.injectExpected) {
    ...
    for (const [yearStr, exp] of Object.entries(expYearData)) {
      const year = parseInt(yearStr)
      if (year < args.since || year > args.until) continue
      perYear[year] = perYear[year] || {}
      const nameGroups = [
      ...
      for (const g of nameGroups) {
        ...
        for (const name of g.list) {
          ...
          if (maybeId) perYear[year][g.cat].add(maybeId)
            ...
            console.warn(
              `Could not resolve BGG id for expected game name '${name}' (${year} ${g.cat})`
            )
        }
      }
    }
    ...
    for (const [cat, idsSet] of Object.entries(catMap)) {
      for (const gid of idsSet) {
        ...
        const honor = buildHonor(
          year,
          cfg.display,
          cat === 'Recommended' ? 'Special' : cat
        )
        const res = await applyHonor(game, honor)
        if (res.action === 'applied')
          console.log(`Applied ${honor.name} to ${game.name}`)
      }
    }
  }
  ...
  Object.entries(report.years).forEach(([y, info]) => {
    ...
    if (v.missing?.length || v.unexpected?.length) {
      console.log(
        `${y}: missing=${v.missing.length} unexpected=${v.unexpected.length}`
      )
    } else {
      console.log(`${y}: OK`)
    }
