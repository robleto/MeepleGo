  ...
  awards: [
    {
      ...
      categories: {
        ...
        special:
          /(beautiful-game|cooperative-family-game|literary-game|dexterity-game|childrens-game|complex-game|fantasy-game|new-worlds-game|party-game|historical-game|special-prize-game|special-prize|game-of-the-ye)/,
      },
      normalizeCategory(slug) {
        // Order: recommended, nominee, plain winner, specials
        if (this.categories && this.categories.recommended.test(slug))
          return { category: 'Recommended' }
        if (this.categories && this.categories.nominee.test(slug))
          return { category: 'Nominee' }
        if (this.categories && this.categories.winner.test(slug))
          return { category: 'Winner' }
        if (this.categories && this.categories.special.test(slug))
