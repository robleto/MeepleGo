...
try {
  ...
  console.error('❌ Failed parsing dataset:', e.message)
  process.exit(1)
}
if (!Array.isArray(dataset)) {
  console.error('❌ Dataset must be array')
  process.exit(1)
}
...
function classify(entry) {
  ...
  if (
    /kinderspiel/.test(slugTitle + position) ||
    /kennerspiel/.test(slugTitle + position)
  )
    return null
  // Winner heuristics: explicit winner slug/title OR position includes 'game of the year'
  if (/winner/.test(slugTitle) || /game of the year/.test(position))
    return 'Winner'
  if (/nominee/.test(slugTitle) || /nominee/.test(position)) return 'Nominee'
  if (/recommend/.test(slugTitle) || /recommend/.test(position))
    return 'Recommended'
  return null // ignore
}
...
for (const entry of dataset) {
  ...
  const boardgames =
    classify(entry) === 'Winner' && entry.boardgames.length > 1
      ? [entry.boardgames[0]]
      : entry.boardgames
  for (const g of boardgames) {
    ...
    if (!rec) {
      rec = []
      perGame.set(g.bggId, rec)
    }
    let existing = rec.find((h) => h.year === entry.year)
    const currentRank =
      cls === 'Recommended' ? precedence['Recommended'] : precedence[cls]
    if (!existing) {
...
function bumpYearStats(year, cls) {
  if (!perYearStats.has(year))
    perYearStats.set(year, { Winner: 0, Nominee: 0, Recommended: 0 })
  const ys = perYearStats.get(year)
  ys[cls]++
}
function downgradeYearStats(year, cls) {
  if (!cls) return
  const ys = perYearStats.get(year)
  if (ys && ys[cls] > 0) ys[cls]--
}
...
byYear.forEach((arr, year) => {
  ...
  const nominees = arr
    .filter((h) => h._label === 'Nominee')
    .slice(0, nomineesAllowed)
  const recs = arr
    .filter((h) => h._label === 'Recommended')
    .slice(0, recommendedCap)
...
// Recompute stats
const summaryRows = Array.from(finalPerYear.entries())
  .sort((a, b) => a[0] - b[0])
  .map(([year, data]) => ({
    year,
...
const finalKept = summaryRows.reduce(
  (sum, r) => sum + r.Winner + r.Nominee + r.Recommended,
  0
)
...
;(async () => {
  ...
  for (const bggId of targetIds) {
    const { data: rows, error } = await supabase
      ...
      .limit(1)
    if (error) {
      console.error('Fetch err', bggId, error.message)
      skipped++
      continue
    }
    if (!rows || !rows.length) {
    ...
    const toAdd = perGame.get(bggId).map((h) => {
      const { _label, _rank, ...rest } = h
      return rest
    })
    ...
    const { error: updErr } = await supabase
      ...
      .eq('id', game.id)
    if (updErr) {
      console.error('Update err', bggId, updErr.message)
      skipped++
      continue
    }
    updated++
  ...
  while (true) {
    ...
    for (const game of rows) {
      ...
      const { error: updErr2 } = await supabase
        ...
        .eq('id', game.id)
      if (updErr2) {
        console.error('Cleanup update error', game.bgg_id, updErr2.message)
        continue
      }
      cleanedGames++
      honorsRemoved += removed
    }
    if (rows.length < batch) break // last page
    from += batch
  }
  console.log(
    'Cleanup complete. Games cleaned:',
    cleanedGames,
    'Honors removed:',
    honorsRemoved
  )
