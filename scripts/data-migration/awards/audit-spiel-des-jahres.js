...
;(async () => {
  ...
  let sourceCount = 0
  for (const h of raw) {
    if (!h.boardgames || !h.boardgames.length || !h.year || !h.awardSet)
      continue
    ...
    for (const g of h.boardgames) {
      ...
      if (!sourceMap.has(key))
        sourceMap.set(key, { categories: new Set(), raw: [] })
      sourceMap.get(key).categories.add(result)
      sourceMap
        .get(key)
        .raw.push({ slug: h.slug, position: h.position, result })
      sourceCount++
    }
  }
  ...
  for (const g of dbGames) {
    ;(g.honors || [])
      .filter((h) => h.award_type === SOURCE_AWARD)
      .forEach((h) => {
        if (typeof h.year !== 'number') return
        const key = `${g.bgg_id}|${h.year}`
        if (!dbMap.has(key))
          dbMap.set(key, { best: h.category, all: new Set([h.category]) })
        else {
          const rec = dbMap.get(key)
          rec.all.add(h.category)
          if (precedence[h.category] > precedence[rec.best])
            rec.best = h.category
        }
        dbAssoc++
      })
  ...
  console.log(
    'Downgraded (DB category lower than source implied):',
    downgraded.length
  )
