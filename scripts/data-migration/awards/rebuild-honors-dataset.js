...
const LIMIT = argVal('--limit', null)
  ? parseInt(argVal('--limit', null), 10)
  : null
...
function detectCategory(title, slug) {
  ...
  if (/nominee|finalist/i.test(basis))
    return { category: 'Nominee', result_raw: 'Nominee' }
  if (/recommended|selection|honorable|inductee|hall of fame/i.test(basis))
    return { category: 'Special', result_raw: 'Special' }
...
function deriveAwardType(awardSet, title, position) {
  ...
  if (position) {
    if (/kinderspiel des jahres/i.test(position))
      return 'Kinderspiel des Jahres'
    if (/kennerspiel des jahres/i.test(position))
      return 'Kennerspiel des Jahres'
    if (
      /spiel des jahres/i.test(position) &&
      !/kinderspiel|kennerspiel/i.test(position)
    )
  ...
  if (title) {
    ...
    if (
      /spiel des jahres/i.test(title) &&
      !/kinderspiel|kennerspiel/i.test(title)
    )
      return 'Spiel des Jahres'
  }
  ...
  if (title) {
    ...
    const t = title
      .replace(/(Winner|Nominee|Finalist|Recommended|Selection|Inductee)$/i, '')
      .trim()
...
function choosePrimaryWinner(honors) {
  ...
  if (honors.length === 1) {
    honors[0].primary_winner = true
    return
  }
  const awardType = honors[0].award_type
  if (MULTI_EQUAL_AWARDS.has(awardType)) {
    honors.forEach((h) => (h.primary_winner = true))
    return
  }
  const byPriority = (h) => {
  ...
  honors.sort(
    (a, b) =>
      ...
      String(a.honor_id).localeCompare(String(b.honor_id))
  )
  honors.forEach((h) => (h.primary_winner = false))
  honors[0].primary_winner = true
}
...
function summarize(groups, normalized, changes) {
  ...
  groups.forEach((arr, key) => {
    ...
    if (MULTI_EQUAL_AWARDS.has(arr[0].award_type)) {
      if (win.length) {
        stats.groupsWithPrimary++
        stats.multiEqualGroups++
      }
    } else {
      if (primaries.length === 1) stats.groupsWithPrimary++
      else if (primaries.length === 0)
        anomalies.missingPrimary.push({ award_type, year, winners: win.length })
      else
        anomalies.multiplePrimary.push({
          award_type,
          year,
  ...
  normalized.forEach((h) => {
    if (h.category === 'Winner') stats.winners++
    else if (h.category === 'Nominee') stats.nominees++
    else stats.specials++
  })
  const subcategoryCoverage =
    normalized.filter((h) => !!h.subcategory).length /
    Math.max(1, normalized.length)
  return {
    stats: {
      ...stats,
      subcategoryCoveragePct: (subcategoryCoverage * 100).toFixed(2),
    },
    anomalies,
...
function main() {
  ...
  let rawArr
  try {
    rawArr = JSON.parse(rawStr)
  } catch (e) {
    console.error('Failed to parse input JSON', e.message)
    process.exit(1)
  }
  if (!Array.isArray(rawArr)) {
    console.error('Input root is not an array')
    process.exit(1)
  }
  const normalized = []
  ...
  for (let i = 0; i < rawArr.length; i++) {
    ...
    const { sub: subcategory, changed: subChanged } = deriveSubcategory(
      r.position,
      award_type
    )
    if (subChanged) changeCounters.standardized_subcategory++
    const { cleaned: cleanedTitle, changed: titleChanged } = cleanTitle(
      r.title || '',
      award_type
    )
    if (titleChanged) changeCounters.cleaned_title++
    const game_refs = (r.boardgames || []).map((bg) => ({
      bgg_id: bg.bggId,
      name: bg.name,
    }))
    normalized.push({
  ...
  groups.forEach((arr) => {
    ...
    if (MULTI_EQUAL_AWARDS.has(awardType))
      changeCounters.multi_equal_primary_groups++
  ...
  fs.writeFileSync(
    ...
    PRETTY ? JSON.stringify(normalized, null, 2) : JSON.stringify(normalized)
  )
  // Detailed report
  fs.writeFileSync(
  ...
  console.log(
    '[rebuild] Anomalies',
    Object.fromEntries(
      Object.entries(report.anomalies).map(([k, v]) => [k, v.length])
    )
  )
