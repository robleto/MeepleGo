...
const exportMissingPath =
  typeof args['export-missing'] === 'string'
    ? args['export-missing']
    : args['export-missing']
      ? path.join(process.cwd(), 'missing-games.json')
      : null
...
if (!supabaseUrl || !supabaseKey) {
  console.error(
    '❌ Missing Supabase credentials (need URL + service role key ideally).'
  )
  process.exit(1)
}
...
console.log(
  `ℹ️  Honors with boardgames usable: ${usableHonors.length} (from ${honorsArray.length} total)`
)
...
function sanitizeHonor(h) {
  ...
  if (h.award_type && typeof h.award_type === 'string')
    out.award_type = h.award_type
...
for (const entry of usableHonors) {
  ...
  for (const game of entry.boardgames) {
    ...
    const honorObj = {
      ...
      category:
        resultType || deriveResultFromPosition(entry.position) || 'Special',
      validated: false,
...
console.log(
  `🧮 Generated ${processedCount} honor-game associations for ${perGame.size} distinct BGG game IDs.`
)
...
if (dryRun) {
  ...
  const sample = Array.from(perGame.entries())
    .slice(0, 5)
    .map(([bgg, honors]) => ({ bgg, count: honors.length, first: honors[0] }))
  ...
  console.log(
    '✅ Dry run complete (no database updates). Use without --dry-run to apply.'
  )
  process.exit(0)
}
...
;(async () => {
  console.log(
    hardReplace
      ? '⚠️  REPLACE mode: existing honors will be overwritten.'
      : '➕ MERGE mode: honors will be merged & de-duplicated.'
  )
  ...
  for (const [bggId, newHonors] of perGame.entries()) {
    ...
    if (fetchErr) {
      console.error(`Fetch error bgg_id=${bggId}:`, fetchErr.message)
      skipped++
      continue
    }
    if (!rows || !rows.length) {
      ...
      if (autoCreateMissing) {
        ...
        const nameFromHonor =
          (newHonors[0] && (newHonors[0].description || newHonors[0].name)) ||
          `BGG ${bggId}`
        const insertPayload = {
          bgg_id: bggId,
          name: nameFromHonor,
        ...
        if (createdGames % 100 === 0)
          console.log(`Created ${createdGames} games...`)
