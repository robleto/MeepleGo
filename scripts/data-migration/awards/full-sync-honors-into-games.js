...
const checkpointPath = path.join(
  process.cwd(),
  'tmp',
  'full-sync-honors.checkpoint.json'
)
  ...
  if (fs.existsSync(checkpointPath)) {
    try {
      ...
      if (typeof cp.lastIndex === 'number') {
        ...
        console.log(
          `Resume enabled. Starting from checkpoint index ${resumeOffset}`
        )
      }
    } catch (e) {
      console.warn(
        'Failed reading checkpoint, falling back to provided start-offset.',
        e.message
      )
    }
  } else {
    console.log(
      'Resume flag set but no checkpoint file found; using provided start-offset.'
    )
  }
}
...
for (const entry of honors) {
  if (!entry || !Array.isArray(entry.boardgames)) {
    skippedNoArray++
    continue
  }
  if (entry.boardgames.length === 0) {
    skippedNoGames++
    continue
  }
...
if (dryRun) {
  const sample = Array.from(perGame.entries())
    .slice(0, 3)
    .map(([id, list]) => ({ bgg: id, count: list.length, first: list[0] }))
...
;(async () => {
  log(
    hardReplace
      ? 'REPLACE mode: overwriting honors arrays.'
      : 'MERGE mode: merging by honor_id.'
  )
  let updated = 0,
    created = 0,
    missing = 0,
  ...
  if (resumeOffset > 0) {
    log(
      `Applying start offset: skipping first ${resumeOffset} games (of ${entries.length})`
    )
  }
  ...
  for (let offset = 0; offset < slicedEntries.length; offset += batchSize) {
    ...
    for (const [bggId, honorsListOriginal] of slice) {
      ...
      const honorsList = honorsListOriginal.filter(
        (h) =>
          h &&
          typeof h.name === 'string' &&
          typeof h.year === 'number' &&
          ['Winner', 'Nominee', 'Special'].includes(h.category)
      )
      ...
      const { data: rows, error: fetchErr } = await supabase
        ...
        .limit(1)
      if (fetchErr) {
        console.error('Fetch error', bggId, fetchErr.message)
        errors++
        continue
      }
      if (!rows || rows.length === 0) {
        if (autoCreateMissing) {
          const insertPayload = {
            bgg_id: bggId,
            name: honorsList[0]?.game_name || `BGG ${bggId}`,
            honors: honorsList,
            created_at: nowIso,
            updated_at: nowIso,
          }
          ...
          if (insErr) {
            console.error('Insert error', bggId, insErr.message)
            errors++
            continue
          }
          created++
        ...
        if (hardReplace) {
          ...
          const existing = Array.isArray(game.honors)
            ? game.honors.filter(
                (h) =>
                  h &&
                  h.name &&
          ...
          for (const h of existing) {
            if (h && h.honor_id) map.set(h.honor_id, h)
          }
          for (const h of honorsList) {
            map.set(h.honor_id, h)
          }
        ...
        const { error: updErr } = await supabase
          ...
          .eq('id', game.id)
        if (updErr) {
          console.error('Update error', bggId, updErr.message)
          errors++
          continue
        }
        updated++
        processed++
        if (!quiet && updated % 200 === 0)
          console.log(`Progress: ${updated} games updated...`)
      }
    }
    const absoluteIndex =
      resumeOffset +
      Math.min(entries.length, resumeOffset + offset + slice.length) -
      1
    // Write checkpoint
    try {
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            lastIndex: resumeOffset + offset + slice.length - 1,
            processed,
            updated,
    ...
    if (!quiet)
      console.log(
        `Batch ${offset / batchSize + 1} complete (processed so far: ${processed}) (last absolute index: ${absoluteIndex})`
      )
  }
  console.log('\n==== FULL SYNC SUMMARY ====')
