...
function extractTitle(slug) {
  ...
  const titleCase = withSpaces.replace(/\b[\w']+/g, (word) => {
    ...
    if (
      word.includes("'") &&
      (word.toLowerCase().includes("d'") || word.toLowerCase().includes("l'"))
    ) {
      return word
    }
    // Standard title case
...
const honorsData = JSON.parse(
  fs.readFileSync('complete-bgg-honors-with-years.json', 'utf8')
)
...
const categories = {
  "As d'Or": honorsWithTitles.filter((h) =>
    h.title.toLowerCase().includes("as d'or")
  ),
  'Tric Trac': honorsWithTitles.filter((h) =>
    h.title.toLowerCase().includes('tric trac')
  ),
  'Spiel des Jahres': honorsWithTitles.filter((h) =>
    h.title.toLowerCase().includes('spiel des jahres')
  ),
  'Golden Geek': honorsWithTitles.filter((h) =>
    h.title.toLowerCase().includes('golden geek')
  ),
  'Origins Awards': honorsWithTitles.filter((h) =>
    h.title.toLowerCase().includes('origins')
  ),
}
...
const asDorHonors = honorsWithTitles.filter(
  (h) =>
    h.title.toLowerCase().includes("as d'or") ||
