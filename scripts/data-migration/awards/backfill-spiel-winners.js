...
async function run() {
  ...
  for (const entry of BACKFILL_WINNERS) {
    ...
    if (!game) {
      console.warn(
        '[backfill-spiel] Game missing in DB for',
        year,
        bgg_id,
        name
      )
    ...
    if (!DRY) {
      const { error } = await supabase
        ...
        .eq('id', game.id)
      if (error) {
        console.error('Update failed for game', bgg_id, error.message)
        continue
      }
    }
    inserted++
    updatedGames++
    console.log(
      `[backfill-spiel] ${DRY ? 'Would insert' : 'Inserted'} winner for ${year}: ${name}`
    )
  }
  console.log('[backfill-spiel] Complete', {
    APPLY,
    inserted,
    skippedExists,
    missingGames,
