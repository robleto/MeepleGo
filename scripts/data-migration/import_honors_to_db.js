...
async function importHonors() {
  ...
  for (const honor of honors) {
    ...
    for (const gameData of honor.boardgames) {
      ...
      if (isNaN(bggId)) {
        console.warn(
          `⚠️  Skipping invalid BGG ID: ${gameData.bggId} for ${gameData.name}`
        )
        continue
      }
  ...
  for (const [bggId, entry] of gameMap.entries()) {
    ...
    if (looksPlaceholder) {
      console.warn(
        `🚫 Skipping insert for BGG ${bggId} due to suspicious placeholder-like name: '${canonical}'`
      )
  ...
  console.log(
    `🛡️  Safeguard: ${gameMap.size} aggregated; ${preparedGames.length} passed name validation; skipped ${gameMap.size - preparedGames.length}.`
  )

  console.log(
    `🎮 Processing ${preparedGames.length} unique games (post-validation)...`
  )
  ...
  for (const gameData of preparedGames) {
    ...
    try {
      ...
      console.error(
        `💥 Unexpected error processing BGG ID ${bggId}:`,
        err.message
      )
      errors++
    }
  ...
  for (const game of artworkGames || []) {
    const artworkHonors = (game.honors || []).filter(
      (h) =>
        h.title &&
        h.title.includes('Golden Geek') &&
  ...
  console.log(
    `🏆 Found ${goldenGeek2024Artwork.length} games in 2024 Golden Geek artwork category:`
  )
