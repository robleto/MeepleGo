...
function buildCanonicalNameMap(honors) {
  ...
  for (const [id, names] of map.entries()) {
    ...
    const realNames = Array.from(names).filter(
      (n) => !/^Golden Geek Best /i.test(n)
    )
...
async function run() {
  console.log(
    `🔧 Repair script starting (mode: ${APPLY ? 'APPLY' : 'DRY-RUN'})`
  )
  ...
  const targetIds2024 = new Set([
    432250, 419496, 404544, 373577, 364733, 403409, 425873, 418542,
  ])
  ...
  const { data: possibleRows, error } = await supabase
    ...
    .or(
      `name.ilike.Golden Geek Best Print and Play Board Game%,bgg_id.in.(${Array.from(targetIds2024).join(',')})`
    )
  ...
  for (const row of possibleRows || []) {
    const isPlaceholderPattern =
      /^Golden Geek Best Print and Play Board Game/i.test(row.name)
    const missingVisuals =
      !row.year_published && !row.image_url && !row.thumbnail_url
    const canonicalName = canonical.get(row.bgg_id)

    // 1. Recover name if placeholder pattern AND have better canonical
    if (
      isPlaceholderPattern &&
      missingVisuals &&
      canonicalName &&
    ...
    if (targetIds2024.has(row.bgg_id) && Array.isArray(row.honors)) {
      ...
      const newHonors = row.honors.map((h) => {
        if (
          h &&
          h.honor_id === printPlayHonorNominee &&
          h.category !== 'Nominee'
        ) {
          h.category = 'Nominee'
          modified = true
        ...
        if (
          h &&
          h.honor_id === printPlayHonorWinner &&
          h.category !== 'Winner'
        ) {
          h.category = 'Winner'
          modified = true
      ...
      if (modified) {
        if (APPLY) {
          const { error: upErr } = await supabase
            ...
            .eq('bgg_id', row.bgg_id)
          if (upErr)
            console.error('❌ Honor update failed', row.bgg_id, upErr.message)
        }
        categoryUpdates++
      }
    }
  ...
  if (renameChanges.length) {
    ...
    renameChanges
      .slice(0, 5)
      .forEach((c) => console.log(`  • ${c.bgg_id}: '${c.from}' -> '${c.to}'`))
  }
  console.log(
    'Honor category updates (110850->Nominee, 111263->Winner):',
    categoryUpdates
  )
