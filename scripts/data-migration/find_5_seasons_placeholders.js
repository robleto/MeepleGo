...
async function fetchCandidates() {
  const { data, error } = await supabase
    .from('games')
    .select(
      'id,bgg_id,name,year_published,min_players,max_players,playtime_minutes,description,image_url,thumbnail_url'
    )
...
async function main() {
  ...
  console.log(
    `Found ${candidates.length} candidate(s). Fetching BGG primary names...`
  )
  const report = []
