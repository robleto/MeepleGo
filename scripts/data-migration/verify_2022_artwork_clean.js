...
;(async () => {
  ...
  for (const game of data) {
    const artworkHonors = (game.honors || []).filter(
      (h) =>
        h.year === 2022 && (h.honor_id === '79656' || h.honor_id === '80063')
    )
  ...
  const nominees = artworkGames.filter((g) =>
    g.honors.some((h) => h.honor_id === '79656')
  )
  const winners = artworkGames.filter((g) =>
    g.honors.some((h) => h.honor_id === '80063')
  )
