...
function buildCanonicalMap(honors) {
  ...
  for (const [id, names] of map.entries()) {
    ...
    const nonAward = all.filter(
      (n) =>
        !/^golden geek/i.test(n) &&
        !/ nominee$/i.test(n) &&
        !/ winner$/i.test(n)
    )
    let chosen = nonAward.length
      ? nonAward.sort((a, b) => b.length - a.length)[0]
      : all.sort((a, b) => b.length - a.length)[0]
...
async function fetchAllGamesBasic() {
  ...
  while (true) {
    const { data, error } = await supabase
      .from('games')
      .select(
        'bgg_id,name,year_published,image_url,thumbnail_url,min_players,max_players,playtime_minutes,description,honors'
      )
...
async function residuePhase(games, canonical) {
  ...
  const restorable = residue.filter(
    (r) =>
      r.canonicalName &&
      r.canonicalName !== r.name &&
      !isPlaceholderName(r.canonicalName)
  )
  const deletable = residue.filter((r) => !restorable.includes(r))
  console.log(
    `Residue scan: ${residue.length} placeholders found. Restorable=${restorable.length} Deletable=${deletable.length}`
  )
  if (APPLY_RESIDUE) {
    for (const r of restorable) {
      const { error } = await supabase
        .from('games')
        .update({ name: r.canonicalName })
        .eq('bgg_id', r.bgg_id)
      if (error) console.error('Restore failed', r.bgg_id, error.message)
      else console.log('Restored', r.bgg_id, '→', r.canonicalName)
    }
    for (const r of deletable) {
      const { error } = await supabase
        ...
        .delete()
        .eq('bgg_id', r.bgg_id)
      if (error) console.error('Delete failed', r.bgg_id, error.message)
      else console.log('Deleted placeholder', r.bgg_id)
    }
  }
...
async function enrichmentPhase(games, honors) {
  ...
  console.log(
    `Metadata: ${needMeta.length} games missing fields. Missing games not in DB: ${missingIds.size}`
  )
      ...
      while (queue.length) {
        ...
        try {
          ...
          if (mode === 'insert') {
            const { error } = await supabase
              .from('games')
              .insert({ bgg_id: id, ...data })
            if (error) console.error('Insert fail', id, error.message)
            else {
              processedCount++
              if (processedCount % 25 === 0)
                console.log(
                  `[insert] ${processedCount} (last ${id} ${data.name})`
                )
            }
          } else {
            const patch = {}
            ...
            if (Object.keys(patch).length) {
              const { error } = await supabase
                ...
                .update(patch)
                .eq('bgg_id', id)
              if (error) console.error('Patch fail', id, error.message)
              else {
                processedCount++
                if (processedCount % 50 === 0)
                  console.log(
                    `[update] ${processedCount} (last ${id} fields: ${Object.keys(patch).join(',')})`
                  )
              }
            }
          }
...
;(async function main() {
  console.log(
    `Honors maintenance starting (residue=${APPLY_RESIDUE ? 'APPLY' : 'scan'} enrich=${APPLY_ENRICH ? 'APPLY' : 'scan'}) using ${HONORS_FILE}`
  )
  ...
  console.log(
    `Restorable: ${residueReport.restorable.length} Deleted (if applied): ${residueReport.deletable.length}`
  )
  console.log(`Metadata missing (pre-update): ${enrichReport.needMeta.length}`)
  console.log(
    `Missing games inserted (if applied): ${enrichReport.missing.length}`
  )
  if (!APPLY_RESIDUE && residueReport.residue.length)
    console.log('Run with --apply-residue to fix placeholders.')
  if (
    !APPLY_ENRICH &&
    (enrichReport.needMeta.length || enrichReport.missing.length)
  )
    console.log(
      'Run with --apply-enrich to patch metadata / add missing games.'
    )
})()
