...
function find2023HeavyHonors(honors) {
  return honors.filter(
    (h) =>
      h.year === 2023 &&
      (h.id === HEAVY_2023_NOMINEE_ID ||
        h.id === HEAVY_2023_WINNER_ID ||
...
async function run() {
  console.log(
    `🏋️ 2023 Golden Geek Heavy repair (mode: ${APPLY ? 'APPLY' : 'DRY-RUN'})`
  )
  ...
  console.log(
    `✅ Found ${heavy2023Honors.length} Heavy honor entries for 2023:`
  )
  heavy2023Honors.forEach((h) => {
    console.log(
      `  • Honor ${h.id}: "${h.title || h.position}" (${h.boardgames?.length || 0} games)`
    )
  })
  ...
  for (const game of games || []) {
    ...
    const isCorrupted =
      /^Golden Geek.*Heavy/i.test(game.name) &&
      !game.year_published &&
      !game.image_url &&
      !game.thumbnail_url
    ...
    if (Array.isArray(game.honors) && mappings.length > 0) {
      const updatedHonors = game.honors.map((honor) => {
        for (const mapping of mappings) {
          if (
            honor?.honor_id === mapping.honorId &&
            honor.category !== mapping.expectedCategory
          ) {
            changes.push({
              bgg_id: game.bgg_id,
              type: 'category',
      ...
      if (
        needsUpdate &&
        JSON.stringify(updatedHonors) !== JSON.stringify(game.honors)
      ) {
    ...
    if (needsUpdate && Object.keys(updates).length > 0 && APPLY) {
      ...
      if (updateError) {
        console.error(
          `❌ Update failed for game ${game.bgg_id}:`,
          updateError.message
        )
      } else {
  ...
  if (changes.length > 0) {
    ...
    changes.forEach((change) => {
      if (change.type === 'name') {
        ...
        console.log(
          `  🏆 ${change.bgg_id}: ${change.from} → ${change.to} (honor ${change.honorId})`
        )
      }
    })
  }
