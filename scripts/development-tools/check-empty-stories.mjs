...
async function scan(dir){
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      if (stripped.length < MIN_NON_WHITESPACE || !/export\s+default\s+/m.test(stripped)) {
        failures.push(full)
      }
    }
  }
...
if (failures.length) {
  console.error(`Empty or invalid story files detected (missing default export or too short):\n` + failures.map(f=>` - ${f}`).join('\n'))
