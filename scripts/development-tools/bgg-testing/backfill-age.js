...
async function backfill(limit = 50) {
  ...
  console.log(
    `Found ${games.length} games missing age (processing up to ${limit}).`
  )
  let updated = 0
  for (const g of games) {
    ...
    if (age) {
      const { error: upErr } = await supabase
        ...
        .update({ age })
        .eq('id', g.id)
      if (upErr) console.error('Update failed', g.bgg_id, upErr.message)
      else {
        updated++
        console.log('Updated age', g.name, age)
      }
