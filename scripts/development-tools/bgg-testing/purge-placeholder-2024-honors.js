...
async function purge() {
  ...
  for (const g of games) {
    ...
    const has2024TargetHonor = honors.some(
      (h) => h && h.year === 2024 && TARGET_AWARDS.has(h.award_type)
    )
  ...
  for (const d of toDelete) {
    const { error: delErr } = await supabase
      ...
      .delete()
      .eq('id', d.id)
    if (delErr) console.error('Delete failed', d.bgg_id, d.name, delErr.message)
    else console.log('Deleted placeholder game:', d.name)
  }
  ...
  for (const u of toUpdate) {
    const { error: upErr } = await supabase
      ...
      .eq('id', u.id)
    if (upErr)
      console.error('Honor strip failed', u.bgg_id, u.name, upErr.message)
    else console.log('Stripped incorrect 2024 honors from:', u.name)
  }

  console.log(
    `Purge summary: deleted ${toDelete.length} placeholder games; updated ${toUpdate.length} legit games.`
  )
  console.log(
    'Next: re-run deterministic 2024 honors fix to ensure only canonical honors exist.'
  )
}
