...
class BGGDynamicRankImporter {
  ...
  async importGamesByBGGRanking(targetCount = 1000, startPage = 1) {
    console.log(
      `🏆 Starting BGG Dynamic Ranking import for ${targetCount} games...`
    )
    console.log(
      `🎯 Starting from BGG browse page ${startPage} and going through ranked games!`
    )
    ...
    while (this.storedCount < targetCount && consecutiveEmptyPages < 3) {
      ...
      if (gameIds.length === 0) {
        ...
        console.log(
          `⚠️ No new games found on page ${currentPage} (${consecutiveEmptyPages}/3 empty pages)`
        )
        currentPage++
    ...
    console.log(
      `🏆 Users will see a comprehensive collection of highly-ranked games!`
    )
  }
}
