...
async function fix2024SpielDesJahres() {
  ...
  console.log(
    'Step 1: Finding games with incorrect 2024 Spiel des Jahres honors...'
  )
  ...
  for (const game of gamesWithIncorrectHonors) {
    const incorrect2024Honors = game.honors.filter(
      (honor) => honor.year === 2024 && honor.award_type === 'Spiel des Jahres'
    )
    ...
    if (incorrect2024Honors.length > 0) {
      ...
      const cleanedHonors = game.honors.filter(
        (honor) =>
  ...
  if (!verifyError) {
    ...
    verifyGames.forEach((game) => {
      const honors2024 = game.honors.filter(
        (h) => h.year === 2024 && h.award_type === 'Spiel des Jahres'
      )
