...
function parseHonorLinksFromHtml(html) {
  ...
  while ((match = honorRegex.exec(html)) !== null) {
    ...
    if (
      slug.includes('spiel-des-jahres') ||
      slug.includes('kennerspiel-des-jahres')
    ) {
      honorLinks.push({
        honor_id: honorId,
        slug: slug,
...
async function findSpielDesJahresHonors() {
  ...
  console.log(
    'Searching BGG honor browse pages for Spiel des Jahres honor IDs...'
  )
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      if (
        ...
        html.includes('Sign In') ||
        html.includes('No results available')
      ) {
      ...
      if (honorLinks.length > 0) {
        console.log(
          `  ✓ Found ${honorLinks.length} Spiel des Jahres related honors`
        )
  ...
  const recentHonors = allHonors.filter(
    (honor) =>
...
async function main() {
  ...
  console.log('\n=== SUMMARY ===')
  console.log(
    `Browse method found: ${browseResults.honors2024.length} 2024 honors`
  )
...
module.exports = {
  findSpielDesJahresHonors,
  tryKnownHonorPatterns,
  parseHonorLinksFromHtml,
}
