...
const GERMAN_AWARDS_COMPLETE = [
  ...
  {
    name: 'Mysterium Kids: Treasures of Poseidon',
    bgg_id: 359867,
    year: 2023,
    category: 'Nominee',
    award_type: 'Kennerspiel des Jahres',
...
async function importGermanAwards() {
  ...
  const spielCount = GERMAN_AWARDS_COMPLETE.filter(
    (g) => g.award_type === 'Spiel des Jahres'
  ).length
  const kennerCount = GERMAN_AWARDS_COMPLETE.filter(
    (g) => g.award_type === 'Kennerspiel des Jahres'
  ).length
  console.log(`\nBreakdown:`)
