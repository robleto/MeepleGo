...
class BGGDynamicRankImporter {
  ...
  async loadExistingGameIds() {
    ...
    const { data, error } = await this.supabase.from('games').select('bgg_id')
    ...
    console.log(
      `✅ Found ${this.existingGameIds.size} existing games in database`
    )
  }
  ...
  async fetchBGGBrowsePage(pageNum) {
    ...
    try {
      ...
      console.error(
        `❌ Error fetching browse page ${pageNum}: ${error.message}`
      )
      return []
    }
  }
  ...
  parseGame(xmlText, gameId) {
    try {
      ...
      // Publisher
      const publisherMatch = xmlText.match(
      ...
      return {
        ...
        publisher: publisherMatch
          ? this.sanitizeXMLText(publisherMatch[1])
          : null,
  ...
  async importGamesByBGGRanking(targetCount = 1500, startPage = 21) {
    console.log(
      `🏆 Starting BGG Dynamic Ranking import for ${targetCount} games (ranks 1001-2500)...`
    )
    console.log(
      `🎯 Starting from BGG browse page ${startPage} and continuing through ranked games!`
    )
    console.log(`📈 This fetches games continuing from rank ~1000 onward!`)
    ...
    while (this.storedCount < targetCount && consecutiveEmptyPages < 5) {
      ...
      if (gameIds.length === 0) {
        ...
        console.log(
          `⚠️ No new games found on page ${currentPage} (${consecutiveEmptyPages}/5 empty pages)`
        )
        currentPage++
      ...
      for (const gameId of gameIds) {
        ...
        try {
          console.log(
            `\n📊 [Page ${currentPage}] Fetching game ${gameId}... (${this.storedCount}/${targetCount})`
          )
          ...
          if (
            !xmlData.includes('<item') ||
            xmlData.includes('Item not found')
          ) {
          ...
          console.log(
            `✅ Parsed: ${game.name} (${game.year_published || 'Unknown'})`
          )
          console.log(
            `   📊 BGG Rank: ${game.rank || 'Unranked'} | Rating: ${game.rating || 'N/A'} | Players: ${game.min_players}-${game.max_players}`
          )
          if (game.summary) {
            console.log(
              `   📝 "${game.summary.substring(0, 80)}${game.summary.length > 80 ? '...' : ''}"`
            )
          }
          if (game.categories?.length > 0) {
            console.log(
              `   🏷️ Categories: ${game.categories.slice(0, 3).join(', ')}${game.categories.length > 3 ? '...' : ''}`
            )
          }
          ...
          if (this.storedCount % 50 === 0) {
            ...
            console.log(
              `⏱️ Rate: ${rate.toFixed(1)} games/sec, ETA: ${Math.round(eta / 60)} minutes`
            )
            console.log(
              `🔄 Processed: ${this.processedCount}, Errors: ${this.errorCount}, Skipped: ${this.skippedCount}`
            )
          ...
          if (this.errorCount > 0 && this.errorCount % 10 === 0) {
            console.log(
              `⚠️ ${this.errorCount} errors encountered, taking a longer break...`
            )
            await new Promise((resolve) => setTimeout(resolve, 20000)) // 20s break
          }
        }

        // Rate limiting - be very respectful to BGG for longer imports
    ...
    console.log(`\n🏆 Dynamic ranking import (1001-2500) completed!`)
    console.log(`📊 Final Results:`)
    console.log(`  • Processed: ${this.processedCount} games`)
    console.log(
      `  • Stored: ${this.storedCount} ranked games (ranks ~1001-${1000 + this.storedCount})`
    )
    ...
    console.log(
      `  • Rate: ${(this.storedCount / totalTime).toFixed(1)} games/sec`
    )
    ...
    // Show some sample games from the latest batch
    const { data: latestGames } = await this.supabase
      .from('games')
    ...
    if (latestGames?.length > 0) {
      console.log(`\n🆕 Latest 10 games added to your collection:`)
      latestGames.forEach((game, index) => {
        console.log(
          `  ${index + 1}. ${game.name} (${game.year_published || 'Unknown'}) - Rank: ${game.rank || 'Unranked'} | Rating: ${game.rating || 'N/A'}`
        )
        if (game.categories?.length > 0) {
    ...
    console.log(
      `\n🚀 Your MeepleGo collection now spans BGG ranks 1-${1000 + this.storedCount}+!`
    )
    console.log(
      `🏆 Users will have access to the most comprehensive ranking-based collection available!`
    )
  }
}

// Create and run the dynamic ranking importer for ranks 1001-2500
const importer = new BGGDynamicRankImporter()
importer.importGamesByBGGRanking(1500, 21).catch(console.error)
