...
function parseFamilyData(xml) {
  try {
    ...
    for (const item of items) {
      ...
      const familyLinks = linkArray.filter(
        (link) => link['@_type'] === 'boardgamefamily'
      )
  ...
  try {
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled' && !result.value.error) {
        ...
        console.error(
          `❌ Failed to update game ${gameData[index].bgg_id}:`,
          error.message
        )
        stats.failures.push({
    ...
    console.log(
      `✅ Database update: ${successful} successful, ${failed} failed`
    )
  } catch (error) {
...
async function importFamilyData() {
  ...
  try {
    ...
    if (
      schemaError &&
      schemaError.message.includes('column "families" does not exist')
    ) {
      console.error(
        '❌ The "families" column does not exist in the games table.'
      )
    ...
    for (let offset = 0; offset < totalCount; offset += FETCH_BATCH_SIZE) {
      console.log(
        `📥 Fetching games ${offset + 1}-${Math.min(offset + FETCH_BATCH_SIZE, totalCount)}...`
      )
    ...
    const gamesWithFamilies = games.filter(
      (g) => g.families && g.families.length > 0
    )
    if (gamesWithFamilies.length > 0) {
      console.log(
        `🔍 Found ${gamesWithFamilies.length} games already have family data`
      )
      const answer = await getUserInput(
        'Skip games that already have family data? (y/n): '
      )
    ...
    console.log(
      `📦 Created ${batches.length} batches of ${CONFIG.BATCH_SIZE} games each\n`
    )
