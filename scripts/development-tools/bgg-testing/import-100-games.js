...
class BGGImporter {
  ...
  async getTopGamesRange(startRank = 1, count = 1000) {
    ...
    while (foundCount < count * 5 && currentId < 500000) {
      // Generate 5x more candidates
  ...
  async importGames(targetCount = 1000) {
    ...
    for (const gameId of gameIds) {
      ...
      try {
        ...
        if (this.errorCount > 0 && this.errorCount % 10 === 0) {
          console.log(
            `⚠️ ${this.errorCount} errors encountered, increasing delay...`
          )
    ...
    if (recentGames?.length > 0) {
      ...
      recentGames.forEach((game, index) => {
        console.log(
          `  ${index + 1}. ${game.name} (${game.year_published || 'Unknown'}) - ${game.rating || 'N/A'}`
        )
        if (game.summary) {
