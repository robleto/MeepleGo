...
async function run(){
  ...
  const list=[...rows.entries()].filter(([id,name])=>/winner|nominee|finalist|recommended|jugong/i.test(name)).map(([bgg_id,name])=>({bgg_id,name})).sort((a,b)=>a.bgg_id-b.bgg_id);
  if(!list.length){console.log('No Japan Boardgame Prize placeholders found.');return;}
