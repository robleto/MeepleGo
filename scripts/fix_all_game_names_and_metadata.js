...
const LIMIT =
  limitArgIndex !== -1 ? parseInt(process.argv[limitArgIndex + 1], 10) : null
const idsArgIndex = process.argv.indexOf('--ids')
const ONLY_IDS =
  idsArgIndex !== -1
    ? new Set(
        process.argv[idsArgIndex + 1]
          .split(',')
...
function isLikelyAwardPlaceholder(name) {
  ...
  if (lower.includes('award') && lower.includes('historians choice'))
    return true
  ...
  if (
    lower.startsWith('spiel der spiele') &&
    /(recommended|empfehl|hit fur|hit mit freunden)/.test(lower)
  )
    return true
  ...
  if (
    (lower.includes('juego del año') || lower.includes('juego del ano')) &&
    /(winner|nominee|finalist|recommended)/.test(lower)
  )
    return true
  ...
  if (
    /gra roku|hra roku|vuoden peli|jocul anului/.test(lower) &&
    /(winner|nominee|finalist|recommended)/.test(lower)
  )
    return true
  ...
  if (
    /^\d{4} (diana jones|japan boardgame prize|games magazine|lucca games|mensa recommended)/.test(
      lower
    )
  )
    return true
  if (
    /diana jones|japan boardgame prize|games magazine|lucca games|mensa recommended/.test(
      lower
    ) &&
    /(winner|nominee|finalist|recommended|select|best)/.test(lower)
  )
    return true
  ...
  if (/^\d{4} (cardboard republic|uk games expo|american tabletop)/.test(lower))
    return true
  if (
    /cardboard republic|uk games expo|american tabletop/.test(lower) &&
    /(award|awards|winner|nominee|finalist|recommended|best)/.test(lower)
  )
    return true
  // Fragment like "Game of" following an award prefix
  if (
    AWARD_PREFIXES.some((p) => lower.startsWith(p)) &&
    / game of$/.test(lower)
  )
    return true
  // Contains 'best' plus 'game' and fairly long -> likely category string
  if (lower.includes('best ') && lower.includes(' game') && lower.length > 30)
    return true
...
async function fetchAllGamesBasic() {
  ...
  while (true) {
    const { data, error } = await supabase
      .from('games')
      .select(
        'id,bgg_id,name,year_published,image_url,thumbnail_url,min_players,max_players,playtime_minutes,description,categories,mechanics,publisher'
      )
...
async function fetchBGG(id) {
  ...
  const mechanics = links
    ...
    .map((l) => l['@_value'])
  const publisher =
    links.find((l) => l['@_type'] === 'boardgamepublisher')?.['@_value'] || null
...
async function main() {
  ...
  const targetGames = ONLY_IDS
    ? games.filter((g) => ONLY_IDS.has(g.bgg_id))
    : games
  const placeholders = []
  const needMeta = []
  ...
  const workSetIds = Array.from(
    new Set([...placeholders, ...needMeta].map((g) => g.bgg_id))
  )
  if (LIMIT) {
  ...
  if (!APPLY) {
    ...
    placeholders
      .slice(0, 10)
      .forEach((p) => console.log(`  ${p.bgg_id}: ${p.name}`))
  ...
  async function worker() {
    while (queue.length) {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          if (attempt < MAX_RETRIES && isRate) {
            ...
            console.warn(
              `Rate limit for ${id}, retry ${attempt + 1}/${MAX_RETRIES} in ${backoff}ms`
            )
          ...
          if (attempt < MAX_RETRIES) {
            ...
            console.warn(
              `Transient error ${id}: ${e.message}. Retry in ${backoff}ms`
            )
            await new Promise((r) => setTimeout(r, backoff))
            continue
          }
      ...
      // Name repair
      if (
        isLikelyAwardPlaceholder(existing.name) &&
        fetched.name &&
      ...
      if (Object.keys(patch).length) {
        const { error } = await supabase
          ...
          .eq('bgg_id', id)
        if (error) {
          console.error('Update fail', id, error.message)
          errors++
        } else {
      ...
      processed++
      if (processed % 50 === 0)
        console.log(`[progress] processed ${processed}/${workSetIds.length}`)
