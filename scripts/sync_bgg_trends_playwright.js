...
const BASE_URL_RAW = process.env.MEEPLEGO_BASE_URL || process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
let BASE_URL = BASE_URL_RAW
async function tryFetch(url, timeoutMs=1500) {
  const ctrl = new AbortController(); const t=setTimeout(()=>ctrl.abort(), timeoutMs)
  try { const res = await fetch(url, { method:'GET', signal: ctrl.signal }); clearTimeout(t); return res } catch(e){ clearTimeout(t); return null }
}
async function detectLocalBase() {
  const candidates = []
  // If NEXT_PUBLIC_APP_URL present, prioritize its origin.
  if (process.env.NEXT_PUBLIC_APP_URL) {
    try { const u = new URL(process.env.NEXT_PUBLIC_APP_URL); candidates.push(u.origin) } catch {}
  }
...
async function verifyBaseUrl() {
  ...
  if (res && res.ok) { BASE_URL = BASE_URL_RAW; return true }
  // Attempt auto-detect only for localhost cases
  if (BASE_URL_RAW.includes('localhost')) {
    console.warn('Configured BASE_URL not reachable, attempting auto-detect of local Next dev port...')
    ...
    console.warn('Local server still not reachable. Set MEEPLEGO_BASE_URL or ensure dev server running.')
    return false
  }
  return false
}
...
const PAGE_LIST_META = {
  bestsellers: {
    ...
    description: 'Weekly snapshot of BoardGameGeek Bestsellers (Top selling titles).',
    list_type: 'bgg_bestsellers'
  },
  hotness: {
    name: 'BGG Hotness',
    description: 'Weekly snapshot of the BGG Hotness list (currently trending titles).',
    list_type: 'bgg_hotness'
  },
  trendingplays: {
    name: 'BGG Trending Plays',
    description: 'Weekly snapshot of games with rapidly increasing play activity.',
    list_type: 'bgg_trendingplays'
  },
  mostplayed: {
    name: 'BGG Most Played',
    description: 'Weekly snapshot of games with the highest recent play counts.',
...
function parseArgs() {
  ...
  for (const a of process.argv.slice(2)) {
    ...
    else if (a.startsWith('--timeout-ms=')) opts.timeoutMs = Math.max(1000, parseInt(a.split('=')[1],10))
...
async function extractPage(context, pageSpec, timeoutMs, debug) {
  ...
  try {
    ...
    await page.goto(pageSpec.url, { waitUntil: 'domcontentloaded', timeout: timeoutMs })
    ...
    try {
      await page.evaluate(async () => {
        await new Promise(r=> { let h = 0; const i = setInterval(()=> { window.scrollTo(0, document.body.scrollHeight); if (document.body.scrollHeight === h) { clearInterval(i); r() } h = document.body.scrollHeight }, 250) })
      })
    } catch {}
    // Wait for at least some anchors
    await page.waitForFunction(() => document.querySelectorAll('a[href*="/boardgame/"]').length > 5, { timeout: timeoutMs }).catch(()=>{})

    const result = await page.evaluate(() => {
      const ordered = []
      const seen = new Set()
      const add = (href) => { const m = /\/boardgame\/(\d+)(?:\/|$)/.exec(href); if (m) { const id = Number(m[1]); if(!seen.has(id)){ seen.add(id); ordered.push(id) } } }
      // Prioritize numbered list items (they hold ranking order) then fallback anchors
      document.querySelectorAll('.numbered-game-list__item a[href*="/boardgame/"]').forEach(a=> add(a.getAttribute('href')||''))
      document.querySelectorAll('a[href*="/boardgame/"]').forEach(a=> add(a.getAttribute('href')||''))
      return { ids: ordered, html: document.documentElement.outerHTML.slice(0,600) }
    })
    ...
    if (debug) {
      ...
      fs.writeFileSync(path.join(dumpDir, `bgg_${pageSpec.label}.html`), fullHtml, 'utf8')
...
async function existingIds(bggIds) {
  ...
  for (let i=0;i<bggIds.length;i+=CHUNK) {
    const { data, error } = await supabase.from('games').select('bgg_id').in('bgg_id', bggIds.slice(i,i+CHUNK))
...
let RUN_LOG_ID = null
const RUN_STATS = {
  pages: {}, total: 0, existing: 0, missing: 0,
  imported: 0, failed: 0, apiUsed: 0, inlineUsed: 0,
  started: Date.now()
}
async function importOne(id, attempt=1) {
  ...
  let res
  try {
    res = await fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ bggId:id }) })
  } catch (e) {
    if (attempt < 2) {
...
function decodeHtmlEntities(text='') {
  return text
    ...
    .replace(/&quot;/g,'"')
    .replace(/&apos;/g,"'")
    .replace(/&#(\d+);/g,(m,c)=>{ try{return String.fromCharCode(parseInt(c,10))}catch{return m}})
    .replace(/&#x([0-9A-Fa-f]+);/g,(m,c)=>{ try{return String.fromCharCode(parseInt(c,16))}catch{return m}})
}
...
async function inlineImport(bggId) {
  ...
  let parsed
  try { parsed = parser.parse(xml) } catch (e) { throw new Error('XML parse fail') }
  const item = parsed?.items?.item
  if (!item) throw new Error('No item')
  // name
  let name=null
  if (Array.isArray(item.name)) { const primary = item.name.find(n=>n['@_type']==='primary'); name = primary?primary['@_value']:item.name[0]['@_value'] }
  ...
  const pick = (t)=> links.filter(l=>l['@_type']===t).map(l=> decodeHtmlEntities(l['@_value'])).filter(Boolean)
  ...
  const pub = links.find(l=>l['@_type']==='boardgamepublisher')?.['@_value']
  const publisher = pub? decodeHtmlEntities(pub):null
  let description = item.description? decodeHtmlEntities(item.description['@_value']||item.description):null
  if (description) description = description.replace(/&amp;#10;|&#10;/g,' ').replace(/\s+/g,' ').trim()
  let summary=null; if (description){ const se = description.indexOf('. '); summary = se>-1? description.slice(0,se+1):description.slice(0,240); if (summary.length>260) summary=summary.slice(0,260)+'…'}
  // expansions / parents
  let parent_bgg_id=null; const expansion_ids_set=new Set()
  links.filter(l=>l['@_type']==='boardgameexpansion' && l['@_id']).forEach(l=>{ const idn=Number(l['@_id']); if(!Number.isFinite(idn)) return; if (l['@_inbound']==='true'){ if(!parent_bgg_id) parent_bgg_id=idn } else expansion_ids_set.add(idn) })
  ...
  const rank_families = rankArray.filter(r=>r['@_type']==='family' && r['@_name']).map(r=> String(r['@_name']).toLowerCase())
  ...
  const upsertRow = {
    bgg_id: Number(bggId), name, year_published, image_url, thumbnail_url,
    categories: categories.length? categories: null,
    mechanics: mechanics.length? mechanics: null,
    min_players, max_players, playtime_minutes, publisher, description, summary,
  ...
  const { error } = await supa.from('games').upsert(upsertRow, { onConflict:'bgg_id' })
...
async function main() {
  ...
  console.log('Env summary: URL='+(process.env.NEXT_PUBLIC_SUPABASE_URL? 'set':'MISSING')+' SERVICE_ROLE='+(process.env.SUPABASE_SERVICE_ROLE_KEY? 'set':'MISSING')+' BGG_LIST_OWNER_ID='+(process.env.BGG_LIST_OWNER_ID? 'set':'MISSING'))
  ...
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    console.warn('[warn] Detected Next.js runtime environment variables; ensure you are executing this script directly with `node scripts/sync_bgg_trends_playwright.js` and not via `next dev`.')
  }
  const t0 = Date.now()
  ...
  if (opts.dryRun) {
    if (missing.length) console.log('[Dry Run] Would import', missing.slice(0,30).join(','), missing.length>30?'...':'')
  ...
  async function worker() {
    while (queue.length) {
      ...
      try {
        ...
        await importOne(id)
        if (beforeApi && API_AVAILABLE) apiUsed++; else inlineUsed++
        imported++; console.log(`✅ ${id} (${imported}/${missing.length})`) 
  ...
  console.log('Imported:', imported, 'Failed:', failed, 'API path:', apiUsed, 'Inline path:', inlineUsed)
...
async function buildPageLists(perPage, opts, { skipWrites=false } = {}) {
  ...
  for (let i=0;i<allArr.length;i+=900) {
    ...
    const { data, error } = await supabase.from('games').select('id,bgg_id').in('bgg_id', slice)
    if (error) { console.error('Mapping fetch error', error.message); continue }
    data?.forEach(r => { if (r.bgg_id) mapping.set(r.bgg_id, r.id) })
  }
  ...
  if (ORDER_COL === 'ranking') base.ranking = rank
    ...
    if (items.length) {
      const { error: insError } = await supabase.from('game_list_items').insert(items)
      if (insError) {
        console.error(`Failed inserting items for ${meta.name}:`, insError.message)
        continue
      }
    }
    ...
    await supabase.from('game_lists').update({ description: meta.description, is_public: true }).eq('id', listId)
...
async function findExistingList(ownerId, list_type) {
  const { data, error } = await supabase
    ...
    .limit(1)
    .maybeSingle()
  if (error) { console.error('[lists] findExistingList error', list_type, error.message); return null }
  return data?.id || null
}
...
async function createList(ownerId, meta) {
  const { data, error } = await supabase
    ...
    .select('id')
    .single()
  if (error) { console.error('[lists] createList error', meta.list_type, error.message); return null }
  return data.id
}
