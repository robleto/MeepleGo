...
async function checkDatabase() {
  ...
  try {
    ...
    console.log(
      `✅ Connected! Games table has ${testData?.[0]?.count || 0} rows`
    )
    ...
    if (schemaError) {
      if (schemaError.message.includes('column "summary" does not exist')) {
        ...
        console.log(
          '🔧 Need to run: ALTER TABLE games ADD COLUMN summary text;'
        )
      } else {
    ...
    if (!gamesError && games) {
      ...
      games.forEach((game) => {
        console.log(
          `  - ${game.name} (${game.year_published || 'Unknown year'})`
        )
        console.log(
          `    BGG ID: ${game.bgg_id}, Summary: ${game.summary ? '✅' : '❌'}`
        )
      })
    }
  } catch (error) {
