...
async function addHonorsColumn() {
  ...
  try {
    ...
    if (
      ...
      testError.message.includes('column "honors" does not exist') ||
      testError.message.includes('column games.honors does not exist')
    ) {
    ...
    console.log(
      "   ALTER TABLE games ADD COLUMN IF NOT EXISTS honors JSONB DEFAULT '[]'::jsonb;"
    )
    console.log('')
    console.log('   -- Add index for performance')
    console.log(
      '   CREATE INDEX IF NOT EXISTS idx_games_honors_gin ON games USING GIN (honors);'
    )
    console.log('')
    console.log("   -- Add constraint to ensure it's an array")
    console.log(
      '   ALTER TABLE games ADD CONSTRAINT IF NOT EXISTS check_honors_is_array'
    )
