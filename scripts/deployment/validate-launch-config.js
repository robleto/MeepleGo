...
async function validateEnvironmentVariables() {
  ...
  const optionalVars = ['SUPABASE_SERVICE_ROLE_KEY', 'NEXT_PUBLIC_AUTH_REDIRECT_BASE']
  for (const envVar of optionalVars) {
    const value = process.env[envVar]
    if (value) {
      logResult('environment', `${envVar}`, true, 'Set correctly')
    } else {
      logResult('environment', `${envVar}`, false, 'Missing (recommended for production)', true)
    }
  }
  ...
  if (supabaseUrl) {
    if (supabaseUrl.includes('.supabase.co')) {
      logResult('environment', 'Supabase URL format', true, 'Valid Supabase URL')
    } else {
      logResult('environment', 'Supabase URL format', false, 'Invalid Supabase URL format', true)
    }
  }
...
async function validateSupabaseConnection() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    logResult('supabase', 'Connection test', false, 'Missing required environment variables')
    console.log('')
    return
  }
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        logResult('supabase', 'Database connection', false, 'Table "games" not found - run setup first', true)
      } else {
        logResult('supabase', 'Database connection', false, `Error: ${error.message}`)
      }
    } else {
      logResult('supabase', 'Database connection', true, 'Successfully connected to database')
    }

    // Test auth configuration
    const { data: session } = await supabase.auth.getSession()
    logResult('supabase', 'Auth client', true, 'Auth client initialized successfully')

  } catch (error) {
    logResult('supabase', 'Connection test', false, `Unexpected error: ${error.message}`)
  }
...
async function validateDNSRecords() {
  ...
  try {
    ...
    if (dmarc && dmarc.includes('v=DMARC1')) {
      logResult('dns', 'DMARC record', true, `Found: ${dmarc.substring(0, 50)}...`)
    } else {
      logResult('dns', 'DMARC record', false, 'DMARC record not found or invalid')
    }

    // Note: DKIM check would require knowing the selector, which varies by provider
    logResult('dns', 'DKIM record', false, 'Cannot check without knowing DKIM selector', true)

  } catch (error) {
    logResult('dns', 'DNS check', false, 'DNS checking failed - install dig tool or check manually')
  }
...
function printNextSteps() {
  ...
  console.log('\n🔗 HELPFUL LINKS')
  console.log('- Supabase Production Guide: docs/deployment/supabase-production-config.md')
  console.log('- DNS Setup Guide: docs/deployment/dns-setup.md')
  console.log('- Environment Variables: docs/deployment/environment-variables.md')
