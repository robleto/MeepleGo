...
class BGGRankOrderImporter {
  ...
  async fetchTopRankedGamesBatch(startRank, batchSize = 100) {
    console.log(
      `🏆 Fetching BGG games ranked ${startRank} to ${startRank + batchSize - 1}...`
    )
    ...
    try {
      ...
      console.log(
        `📡 Fetching from BGG browse page: ${Math.ceil(startRank / 100)}`
      )
      ...
      console.log(
        `🎯 Found ${newGames.length} new games in rank range ${startRank}-${startRank + batchSize - 1}`
      )
      return newGames
    } catch (error) {
  ...
  async importGamesByRank(targetCount = 2000, startRank = 1) {
    console.log(
      `🏆 Starting BGG Rank-Ordered import for ${targetCount} games...`
    )
    console.log(
      `🎯 Starting from BGG rank #${startRank} and going down the list!`
    )
    ...
    while (this.storedCount < targetCount) {
      ...
      const gameIds = await this.fetchTopRankedGamesBatch(
        currentRank,
        batchSize
      )
      ...
      if (gameIds.length === 0) {
        console.log(
          `⚠️ No more ranked games available starting from rank ${currentRank}`
        )
        break
      }
    ...
    console.log(
      `🏆 Users will see the absolute best games first - perfect for ranking and playing!`
    )
  }
}
