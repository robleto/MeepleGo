...
async function fetchGame(bggId) {
  try {
    ...
    if (!res.ok) {
      console.error('BGG fetch http error', bggId, res.status)
      return null
    }
    ...
    const collect = (type) => {
      ...
      const out = new Set()
      let m
      while ((m = r.exec(xml)) !== null) out.add(decode(m[1]))
    ...
    return {
      bgg_id: bggId,
      name:
        decode(get(/<name[^>]*type="primary"[^>]*value="([^"]+)"/)) ||
        `Game ${bggId}`,
      year_published:
        parseInt(get(/<yearpublished[^>]*value="([0-9]+)"/)) || null,
      ...
      playtime_minutes:
        parseInt(get(/<playingtime[^>]*value="([0-9]+)"/)) || null,
...
async function ensureGame(bgg_id) {
  ...
  if (error && error.code !== 'PGRST116') {
    console.error('Error querying game', bgg_id, error.message)
    return null
  }
  const full = await fetchGame(bgg_id)
  if (!full) {
    console.error('Skipping insertion; fetch failed', bgg_id)
    return null
  }
  ...
  if (insertErr) {
    console.warn(
      'Initial insert failed, attempting progressive field stripping:',
      insertErr.message
    )
    ...
    if (insertErr) {
      console.error('Final insert failure for', bgg_id, insertErr.message)
      return null
    }
  }
...
async function removeIncorrect2024Honors() {
  ...
  for (const g of games) {
    ...
    const filtered = honors.filter(
      (h) => !(h && h.year === 2024 && targetAwards.has(h.award_type))
    )
...
async function apply2024() {
  ...
  queue.push({
    ...SPIEL_2024.winner,
    award_type: SPIEL_2024.award_type,
    year: SPIEL_2024.year,
    category: 'Winner',
  })
  SPIEL_2024.nominees.forEach((n) =>
    queue.push({
      ...n,
      award_type: SPIEL_2024.award_type,
      year: SPIEL_2024.year,
  ...
  SPIEL_2024.recommended.forEach((r) =>
    queue.push({
      ...r,
      award_type: SPIEL_2024.award_type,
      year: SPIEL_2024.year,
  ...
  queue.push({
    ...KENNER_2024.winner,
    award_type: KENNER_2024.award_type,
    year: KENNER_2024.year,
    category: 'Winner',
  })
  KENNER_2024.nominees.forEach((n) =>
    queue.push({
      ...n,
      award_type: KENNER_2024.award_type,
      year: KENNER_2024.year,
  ...
  for (const entry of queue) {
    ...
    const exists = honors.some(
      (h) =>
        h &&
        h.year === entry.year &&
        h.award_type === entry.award_type &&
    ...
    if (!exists) {
      ...
      if (upErr) {
        ...
        applied++
        console.log(
          `Applied ${entry.year} ${entry.award_type} ${entry.category} to ${game.name}`
        )
      }
