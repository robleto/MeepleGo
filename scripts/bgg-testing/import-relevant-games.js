...
class BGGHotListImporter {
  ...
  async fetchTopRankedGames(startRank = 1, endRank = 100) {
    console.log(
      `🏆 Fetching Top ${endRank} BGG games (ranks ${startRank}-${endRank})...`
    )
    ...
    const newGameIds = [...popularGameIds, ...recentPopularIds].filter(
      (id) => !this.existingGameIds.has(id)
    )
  ...
  async buildGameQueue(targetCount = 2500) {
    console.log(
      `🎯 Building queue of relevant games for ${targetCount} imports...`
    )
  ...
  async importRelevantGames(targetCount = 2500) {
    console.log(
      `🚀 Starting BGG Hot/Popular Games import for ${targetCount} relevant games...`
    )
    ...
    for (const gameId of gameIds) {
      ...
      try {
        ...
        console.log(
          `   🎯 Rating: ${game.rating || 'N/A'} | Rank: ${game.rank || 'Unranked'} | Players: ${game.min_players}-${game.max_players}`
        )
        if (game.summary) {
          console.log(
        ...
        if (this.errorCount > 0 && this.errorCount % 10 === 0) {
          console.log(
            `⚠️ ${this.errorCount} errors encountered, taking a break...`
          )
    ...
    if (recentGames?.length > 0) {
      ...
      recentGames.forEach((game, index) => {
        console.log(
          `  ${index + 1}. ${game.name} (${game.year_published || 'Unknown'}) - Rating: ${game.rating || 'N/A'} | Rank: ${game.rank || 'Unranked'}`
        )
        if (game.categories?.length > 0) {
    ...
    console.log(
      `🎮 Perfect for playing, ranking, and discovering new favorites!`
    )
  }
}
