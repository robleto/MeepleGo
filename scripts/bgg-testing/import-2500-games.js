...
class BGGImporter {
  ...
  async getMoreGamesRange(targetCount = 2500) {
    console.log(
      `📡 Generating candidate game IDs for ${targetCount} additional games...`
    )
    ...
    while (foundCount < targetCount * 3 && currentId < 600000) {
      // Generate 3x more candidates
  ...
  async importGames(targetCount = 2500) {
    ...
    console.log(
      `🔍 Will check ${gameIds.length} candidate game IDs for ${targetCount} new games`
    )
    ...
    for (const gameId of gameIds) {
      ...
      try {
        ...
        if (game.summary) {
          console.log(
            `   Summary: "${game.summary.substring(0, 100)}${game.summary.length > 100 ? '...' : ''}"`
          )
        }
        console.log(
          `   Rating: ${game.rating || 'N/A'}, Players: ${game.min_players}-${game.max_players}, Time: ${game.playtime_minutes}min`
        )
    ...
    console.log(
      `  • Time: ${Math.round(totalTime / 60)} minutes (${Math.round(totalTime)} seconds)`
    )
    console.log(
    ...
    if (recentGames?.length > 0) {
      ...
      recentGames.forEach((game, index) => {
        console.log(
          `  ${index + 1}. ${game.name} (${game.year_published || 'Unknown'}) - Rating: ${game.rating || 'N/A'}`
        )
        if (game.categories?.length > 0) {
          console.log(
            `     Categories: ${game.categories.slice(0, 3).join(', ')}${game.categories.length > 3 ? '...' : ''}`
          )
        }
        if (game.summary) {
          console.log(
    ...
    console.log(
      `🎮 Ready to explore ${finalCount} board games with advanced filtering!`
    )
  }
}
