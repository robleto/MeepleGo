...
async function clearErroneousGameData() {
  ...
  console.log(
    'This will remove games that were imported with incorrect data...\n'
  )
  ...
  try {
    ...
    recentGames.forEach((game) => {
      console.log(
        `  ${game.id}: ${game.name} (${game.year_published}) - BGG: ${game.bgg_id}`
      )
    })
    ...
    honors2024.forEach((honor) => {
      console.log(
        `  Honor ${honor.id}: Game ${honor.game_id} - ${honor.award} ${honor.year} (${honor.category})`
      )
    })
    ...
    const { data: remainingGames } = await supabase.from('games').select('id')

    const { data: remainingHonors } = await supabase.from('honors').select('id')

    console.log(`Games remaining: ${remainingGames?.length || 0}`)
    console.log(`honors remaining: ${remainingHonors?.length || 0}`)

    if (
      (remainingGames?.length || 0) === 0 &&
      (remainingHonors?.length || 0) === 0
    ) {
      console.log('\n✅ CLEANUP COMPLETE')
      console.log(
        'Both games and honors tables are now empty and ready for fresh data import.'
      )
    } else {
