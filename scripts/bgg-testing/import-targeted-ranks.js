...
class BGGTargetedRankImporter {
  ...
  generateTargetGameIds(count = 2000) {
    ...
    for (const range of ranges) {
      ...
      for (let i = 0; i < range.samples; i++) {
        const randomId =
          Math.floor(Math.random() * (range.end - range.start)) + range.start
        if (
          !this.existingGameIds.has(randomId) &&
          !rangeIds.includes(randomId)
        ) {
  ...
  async importTargetedRankedGames(targetCount = 1500) {
    console.log(
      `🎯 Starting targeted import for ${targetCount} games with ranks 1001-2500...`
    )
    ...
    for (const gameId of gameIds) {
      ...
      try {
        ...
        console.log(
          `   🏆 BGG Rank: ${game.rank} | Rating: ${game.rating || 'N/A'} | Players: ${game.min_players}-${game.max_players}`
        )
        if (game.summary) {
          console.log(
        ...
        console.log(
          `💾 Stored: ${storedGame.name} (${this.storedCount}/${targetCount})`
        )
        ...
        if (this.storedCount % 25 === 0) {
          ...
          console.log(
            `⏱️ Rate: ${rate.toFixed(2)} games/sec, ETA: ${Math.round(eta / 60)} minutes`
          )
          console.log(
    ...
    console.log(
      `  • Stored: ${this.storedCount} ranked games (ranks 1001-2500)`
    )
    console.log(
      `  • Skipped: ${this.skippedCount} (duplicates/not ranked/outside range)`
    )
    console.log(`  • Errors: ${this.errorCount}`)
    console.log(`  • Time: ${Math.round(totalTime / 60)} minutes`)
    console.log(
      `  • Rate: ${(this.storedCount / totalTime).toFixed(2)} games/sec`
    )
    ...
    if (rankData?.length > 0) {
      ...
      console.log(
        `🥇 New rank range: ${rankData[0].rank} to ${rankData[rankData.length - 1].rank}`
      )
    ...
    console.log(
      `🏆 Users can explore and rank games across the full spectrum of board game quality!`
    )
  }
}
