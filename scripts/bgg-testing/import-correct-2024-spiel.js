...
async function getBggGameData(gameId) {
  try {
    ...
    const playersMatch = xmlText.match(
      /<minplayers[^>]*value="([^"]+)"[\s\S]*?<maxplayers[^>]*value="([^"]+)"/
    )
...
async function ensureGameWithHonor(bggId, gameData, honorData) {
  ...
  const honorExists = currentHonors.some(
    (honor) =>
...
const CORRECT_2024_SPIEL_DATA = [
  ...
  },
  {
    ...
    year: 2024,
    category: 'Special',
    name: 'The Animals of Baker Street',
  },
]
...
async function importCorrect2024Data() {
  ...
  const allGames = [
    ...CORRECT_2024_SPIEL_DATA,
    ...CORRECT_2024_KENNERSPIEL_DATA,
  ]
  ...
  for (const gameEntry of allGames) {
    ...
    try {
      ...
      console.log(
        `  ✅ Success: ${gameEntry.name} - ${gameEntry.award} ${gameEntry.year} (${gameEntry.category})`
      )
      successCount++
