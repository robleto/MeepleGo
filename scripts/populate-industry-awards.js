  ...
  try {
    ...
    for (let i = 0; i < honors.length; i += batchSize) {
        ...
        try {
          ...
          if (!honor.boardgames || honor.boardgames.length === 0) {
            ...
            if (processedCount % 500 === 0) {
              console.log(`⚪ [${processedCount}/${honors.length}] Skipped (no games): ${honor.title}`);
            }
            continue;
          }
          ...
          const { data: awardData, error: awardError } = await supabase
            ...
            .insert({
              ...
              alternate_names: honor.alternateNames && honor.alternateNames.length > 0 ? honor.alternateNames : null,
          ...
          if (awardError) {
            if (awardError.code === '23505') { // Unique constraint violation
              console.log(`⚪ [${processedCount}/${honors.length}] Already exists: ${honor.title} (${honor.id})`);
              continue;
            }
            throw awardError;
          }
          ...
          console.log(`➕ [${processedCount}/${honors.length}] Added: ${honor.title} (${honor.year || 'No year'}) with ${honor.boardgames.length} games`);
      ...
      if (i % (batchSize * 10) === 0 && i > 0) {
        console.log(`📈 Progress: ${Math.min(i + batchSize, honors.length)}/${honors.length} (${Math.round((i/honors.length) * 100)}%)`);
